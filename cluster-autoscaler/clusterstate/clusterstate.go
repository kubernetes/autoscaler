...
// RegisterOrUpdateScaleUp registers scaleup for give node group or changes requested node increase
// count.
...
// left intact.
func (csr *ClusterStateRegistry) RegisterOrUpdateScaleUp(nodeGroupName string, increaseDelta int) {
	csr.Lock()
	defer csr.Unlock()

	scaleUpRequest, found := csr.scaleUpRequests[nodeGroupName]
	if !found {
		scaleUpRequest = &ScaleUpRequest{
			NodeGroupName:   nodeGroupName,
			Time:            time.Now(),
			ExpectedAddTime: time.Now().Add(csr.config.MaxNodeProvisionTime),
		}
		csr.scaleUpRequests[nodeGroupName] = scaleUpRequest
	}
	...
	if increaseDelta > 0 {
		...
		scaleUpRequest.Time = time.Now()
		scaleUpRequest.ExpectedAddTime = time.Now().Add(csr.config.MaxNodeProvisionTime)
	}
}
