...
func (az *Cloud) safeDeleteLoadBalancer(lb network.LoadBalancer, clusterName, vmSetName string, service *v1.Service) *retry.Error {
	lbBackendPoolID := az.getBackendPoolID(pointer.StringDeref(lb.Name, ""), az.getLoadBalancerResourceGroup(), getBackendPoolName(clusterName, service))
	...
	if wantLb && nodes != nil && !isBackendPoolPreConfigured {
		...
		if lb.LoadBalancerPropertiesFormat != nil && lb.BackendAddressPools != nil {
			...
			for _, backendPool := range backendPools {
				if strings.EqualFold(pointer.StringDeref(backendPool.Name, ""), getBackendPoolName(clusterName, service)) {
...
func serviceOwnsPublicIP(service *v1.Service, pip *network.PublicIPAddress, clusterName string) (bool, bool) {
	...
	if pip.Tags != nil {
		...
		if serviceTag == "" {
			return isServiceSelectPIP(service, pip), true
		}
		...
		return isServiceSelectPIP(service, pip), false
	}
...
func isServicePIPNameMatchesPIP(service *v1.Service, pip *network.PublicIPAddress) bool {
	return strings.EqualFold(pointer.StringDeref(pip.Name, ""), getServicePIPName(service))
}

func isServiceSelectPIP(service *v1.Service, pip *network.PublicIPAddress) bool {
	return isServiceLoadBalancerIPMatchesPIP(service, pip) || isServicePIPNameMatchesPIP(service, pip)
}
