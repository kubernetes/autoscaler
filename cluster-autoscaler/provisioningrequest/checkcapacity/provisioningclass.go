...
func (o *checkCapacityProvClass) Provision(
	...
	for len(prPods) > 0 {
		...
		if stopBatch := o.shouldStopBatchProcessing(provisioningRequestsProcessed, startTime); stopBatch {
			break
		}
...
func (o *checkCapacityProvClass) checkcapacity(unschedulablePods []*apiv1.Pod, provReq *provreqwrapper.ProvisioningRequest) (bool, error) {
	...
	if len(st) < len(unschedulablePods) || err != nil {
		...
		err := o.context.ClusterSnapshot.Commit()
		if err != nil {
			return false, fmt.Errorf("failed to commit cluster snapshot: %v", err)
		}
	...
	_, err = o.client.UpdateProvisioningRequest(provReq.ProvisioningRequest)
	if err != nil {
		return false, fmt.Errorf("failed to update Provisioned condition to ProvReq %s/%s, err: %v", provReq.Namespace, provReq.Name, err)
	}
...
func (o *checkCapacityProvClass) shouldStopBatchProcessing(prsProcessed int, startTime time.Time) bool {
	if !o.checkCapacityBatchProcessing || prsProcessed >= o.checkCapacityProvisioningRequestMaxBatchSize {
		return true
	}
...
func (o *checkCapacityProvClass) getNextPrPods() ([]*apiv1.Pod, error) {
