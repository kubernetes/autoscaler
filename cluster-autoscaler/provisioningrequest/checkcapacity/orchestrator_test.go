...
func TestScaleUp(t *testing.T) {
	...
	newCpuProvReq := provreqwrapper.BuildTestProvisioningRequest("ns", "newCpuProvReq", "5m", "5", "", int32(100), false, time.Now(), CheckCapacityClass)
	newMemProvReq := provreqwrapper.BuildTestProvisioningRequest("ns", "newMemProvReq", "1m", "100", "", int32(100), false, time.Now(), CheckCapacityClass)
	bookedCapacityProvReq := provreqwrapper.BuildTestProvisioningRequest("ns", "bookedCapacity", "1m", "200", "", int32(100), false, time.Now(), CheckCapacityClass)
	bookedCapacityProvReq.SetConditions([]metav1.Condition{{Type: string(BookCapacityCondition), Status: metav1.ConditionTrue, LastTransitionTime: metav1.Now()}})
	expiredProvReq := provreqwrapper.BuildTestProvisioningRequest("ns", "bookedCapacity", "1m", "200", "", int32(100), false, time.Now(), CheckCapacityClass)
	expiredProvReq.SetConditions([]metav1.Condition{{Type: string(ExpiredCondition), Status: metav1.ConditionTrue, LastTransitionTime: metav1.Now()}})
	differentProvReqClass := provreqwrapper.BuildTestProvisioningRequest("ns", "differentProvReqClass", "1", "1", "", int32(5), false, time.Now(), "AnotherProvisioningClass")
	testCases := []struct {
		name             string
