...
func TestStaticAutoscaler_FullLifecycle(t *testing.T) {
	fakeK8s := fakek8s.NewKubernetes()
	fakeCloud := fakecloudprovider.NewCloudProvider(
		fakecloudprovider.WithKubeClient(fakeK8s),
	)

	testConfig := integration.NewBuilder().
		WithCloudProvider(fakeCloud).
		WithKubeClient(fakeK8s.KubeClient).
		WithAutoscalingOptions(
			integration.DefaultAutoscalingOptions,
			integration.WithCloudProviderName("gce"),
			integration.WithScaleDownUnneededTime(unneededTime),
			integration.WithScaleDownDelayAfterAdd(0),
		).Build()
	...
	integration.RunTest(t, testConfig, func(ctx *integration.TestContext) {
		...
		fakeK8s.RemovePod(p2.Namespace, p2.Name)
...
func TestScaleUp_ResourceLimits(t *testing.T) {
	fakeK8s := fakek8s.NewKubernetes()
	fakeCloud := fakecloudprovider.NewCloudProvider(fakecloudprovider.WithKubeClient(fakeK8s))
