...
echo "Identified ${#MIG_LIST[@]} distinct MIG(s): ${MIG_LIST[*]}"

# Default limits per MIG for parallel workers (1 to 5 nodes per MIG)
...
# Export discovered MIGs for downstream test runner processes
export E2E_MIG_NAMES="$(IFS=,; echo "${MIG_LIST[*]}")"
