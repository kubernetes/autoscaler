...
func NewProvReqProcessor(client *provreqclient.ProvisioningRequestClient, checkCapacityProvisioningClassProcessorInstance string) *provReqProcessor {
	return &provReqProcessor{now: time.Now, maxUpdated: defaultMaxUpdated, client: client, injector: scheduling.NewHintingSimulator(), checkCapacityProvisioningClassProcessorInstance: checkCapacityProvisioningClassProcessorInstance}
}
...
func (p *provReqProcessor) refresh(provReqs []*provreqwrapper.ProvisioningRequest) {
	...
	for _, provReq := range provReqs {
		...
		if !provisioningrequest.SupportedProvisioningClass(provReq.ProvisioningRequest, p.checkCapacityProvisioningClassProcessorInstance) {
			continue
		}
...
func (p *provReqProcessor) bookCapacity(ctx *context.AutoscalingContext) error {
	...
	for _, provReq := range provReqs {
		if !conditions.ShouldCapacityBeBooked(provReq, p.checkCapacityProvisioningClassProcessorInstance) {
			continue
		}
