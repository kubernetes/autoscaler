...
func TestProvisioningRequestPodsInjector(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			provReqs: []*provreqwrapper.ProvisioningRequest{newProvReqA, provisionedAcceptedProvReqB},
			checkCapacityProvisioningClassProcessorInstance: "test-instance",
		},
		{
		...
		},
		{
			...
			provReqs: []*provreqwrapper.ProvisioningRequest{newProvReqA, provisionedAcceptedProvReqB},
			checkCapacityProvisioningClassProcessorInstance: "test-prefix.",
		},
		{
	...
	for _, tc := range testCases {
		...
		injector := ProvisioningRequestPodsInjector{1 * time.Minute, 10 * time.Minute, backoffTime, clock.NewFakePassiveClock(now), client, now, tc.checkCapacityBatchProcessing, tc.checkCapacityProvisioningClassProcessorInstance}
