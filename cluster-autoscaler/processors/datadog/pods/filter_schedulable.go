...
func (p *filterOutSchedulablePodListProcessor) filterOutSchedulableByPacking(
	...
	unschedulablePodsCache := scheduling.NewSimilarPodsScheduling()
	...
	for _, pod := range podsToCheckAgainstAllNodes {
		found := unschedulablePodsCache.IsSimilarUnschedulable(pod)
		if found {
		...
		if err == nil {
			...
			unschedulablePodsCache.SetUnschedulable(pod)
		}
	}
