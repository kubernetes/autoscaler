...
func TestDefaultProcessorFilterOut(t *testing.T) {
	...
	for tcName, tc := range testCases {
		t.Run(tcName, func(t *testing.T) {
			...
			resultedAllNodes, resultedReadyNodes := processor.FilterOutNodesWithUnreadyResources(nil, tc.allNodes, readyNodes, drasnapshot.Snapshot{})
			assert.True(t, len(resultedAllNodes) == len(tc.allNodes))
...
func TestDefaultProcessorGetNodeResourceTargets(t *testing.T) {
	...
	testCases := map[string]struct {
		node                                    *apiv1.Node
		expectedCustomResourceTargets           []string
		expectedCustomResourceTargetsQuantities []int64
	}{
		"single processor": {
			node:                                    utils.BuildTestNode("p1", 500, 100),
			expectedCustomResourceTargets:           []string{"p1_R1", "p1_R2"},
			expectedCustomResourceTargetsQuantities: []int64{1, 1},
		},
		"many processors": {
			node:                                    utils.BuildTestNode("p1_p3", 500, 100),
			expectedCustomResourceTargets:           []string{"p1_R1", "p1_R2", "p3_R1"},
			expectedCustomResourceTargetsQuantities: []int64{1, 1, 3},
		},
		"all processors": {
			node:                                    utils.BuildTestNode("p1_p2_p3", 500, 100),
			expectedCustomResourceTargets:           []string{"p1_R1", "p1_R2", "p2_R1", "p3_R1"},
			expectedCustomResourceTargetsQuantities: []int64{1, 1, 2, 3},
		},
	}
	for tcName, tc := range testCases {
		t.Run(tcName, func(t *testing.T) {
			customResourceTarget, _ := processor.GetNodeResourceTargets(nil, tc.node, nil)
			assert.True(t, len(customResourceTarget) == len(tc.expectedCustomResourceTargets))
			expectedResources := map[string]int64{}
			for index, rt := range tc.expectedCustomResourceTargets {
				expectedResources[rt] = tc.expectedCustomResourceTargetsQuantities[index]
			}
			for _, rt := range customResourceTarget {
				count, found := expectedResources[rt.ResourceType]
				assert.True(t, found)
				assert.True(t, count == rt.ResourceCount)
			}
		})
	}
}
...
func (m *mockCustomResourcesProcessor) FilterOutNodesWithUnreadyResources(context *context.AutoscalingContext, allNodes, readyNodes []*apiv1.Node, draSnapshot drasnapshot.Snapshot) ([]*apiv1.Node, []*apiv1.Node) {
...
func (m *mockCustomResourcesProcessor) GetNodeResourceTargets(context *context.AutoscalingContext, node *apiv1.Node, nodeGroup cloudprovider.NodeGroup) ([]CustomResourceTarget, errors.AutoscalerError) {
