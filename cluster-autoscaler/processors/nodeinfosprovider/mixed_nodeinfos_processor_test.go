...
func TestGetNodeInfosForGroups(t *testing.T) {
	...
	res, err := NewMixedNodeInfoProviderProcessor().Process([]*apiv1.Node{unready4, unready3, ready2, ready1}, &ctx, []*appsv1.DaemonSet{}, nil)
	assert.NoError(t, err)
	...
	res, err = NewMixedNodeInfoProviderProcessor().Process([]*apiv1.Node{}, &ctx, []*appsv1.DaemonSet{}, nil)
	assert.NoError(t, err)
...
func TestGetNodeInfosForGroupsCache(t *testing.T) {
	...
	niProcessor := NewMixedNodeInfoProviderProcessor()
