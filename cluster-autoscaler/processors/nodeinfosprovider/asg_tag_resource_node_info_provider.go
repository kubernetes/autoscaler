...
func NewAsgTagResourceNodeInfoProvider() *AsgTagResourceNodeInfoProvider {
	return &AsgTagResourceNodeInfoProvider{
		mixedTemplateNodeInfoProvider: NewMixedTemplateNodeInfoProvider(),
	}
}

// Process returns the nodeInfos set for this cluster.
func (p *AsgTagResourceNodeInfoProvider) Process(ctx *context.AutoscalingContext, nodes []*apiv1.Node, daemonsets []*appsv1.DaemonSet, ignoredTaints taints.TaintKeySet) (map[string]*schedulerframework.NodeInfo, errors.AutoscalerError) {
	nodeInfos, err := p.mixedTemplateNodeInfoProvider.Process(ctx, nodes, daemonsets, ignoredTaints)
	if err != nil {
		return nil, err
	}
