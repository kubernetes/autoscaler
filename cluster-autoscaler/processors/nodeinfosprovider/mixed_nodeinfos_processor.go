...
// MixedNodeInfoProviderProcessor build nodeInfos from the cluster's nodes and node groups.
type MixedNodeInfoProviderProcessor struct {
	nodeInfoCache map[string]*schedulerframework.NodeInfo
}

// NewMixedNodeInfoProviderProcessor returns a NodeInfoProvider processor building
// NodeInfos from real-world nodes when available, otherwise from node groups templates.
func NewMixedNodeInfoProviderProcessor() *MixedNodeInfoProviderProcessor {
	return &MixedNodeInfoProviderProcessor{
		nodeInfoCache: make(map[string]*schedulerframework.NodeInfo),
	}
}

// CleanUp cleans up processor's internal structures.
func (p *MixedNodeInfoProviderProcessor) CleanUp() {
}

// Process returns the nodeInfos set for this cluster
func (p *MixedNodeInfoProviderProcessor) Process(nodes []*apiv1.Node, ctx *context.AutoscalingContext, daemonsets []*appsv1.DaemonSet, ignoredTaints taints.TaintKeySet) (map[string]*schedulerframework.NodeInfo, errors.AutoscalerError) {
