...
// Autoscaler is the builder object for creating a Cluster Autoscaler instance.
...
// NewAutoscaler creates a builder with default options.
func NewAutoscaler(opts config.AutoscalingOptions) *Autoscaler {
	return &Autoscaler{
...
func (b *Autoscaler) WithKubeClient(client kubernetes.Interface) *Autoscaler {
	b.kubeClient = client
	return b
}

// WithListerRegistry allows injecting a fake ListerRegistry.
func (b *Autoscaler) WithListerRegistry(registry kube_util.ListerRegistry) *Autoscaler {
	b.listerRegistry = registry
	return b
}

// WithPodObserver allows injecting a pod observer.
func (b *Autoscaler) WithPodObserver(podObserver *loop.UnschedulablePodObserver) *Autoscaler {
	b.podObserver = podObserver
	return b
}

// WithCloudProvider allows injecting a cloud provider.
func (b *Autoscaler) WithCloudProvider(cloudProvider cloudprovider.CloudProvider) *Autoscaler {
	b.cloudProvider = cloudProvider
	return b
}

// Build constructs the Autoscaler based on the provided configuration.
func (b *Autoscaler) Build(ctx context.Context, debuggingSnapshotter debuggingsnapshot.DebuggingSnapshotter) (core.Autoscaler, *loop.LoopTrigger, error) {
	// Get AutoscalingOptions from flags.
	autoscalingOptions := b.autoscalingOptions
	...
	informerFactory := informers.NewSharedInformerFactoryWithOptions(b.kubeClient, 0, informers.WithTransform(trim))

	fwHandle, err := framework.NewHandle(ctx, informerFactory, autoscalingOptions.SchedulerConfig, autoscalingOptions.DynamicResourceAllocationEnabled, autoscalingOptions.CSINodeAwareSchedulingEnabled)
	if err != nil {
		return nil, nil, err
	}
	...
	// Create autoscaler.
	autoscaler, err := core.NewAutoscaler(ctx, opts, informerFactory)
	if err != nil {
		return nil, nil, err
	}
	...
	trigger := loop.NewLoopTrigger(autoscaler, ProvisioningRequestInjector, b.podObserver, b.autoscalingOptions.ScanInterval)
