...
func TestDefaultRBACUpdater(t *testing.T) {
	...
	// Verify NO ClusterRole was created (it now only logs)
	_, err = fakeK8s.RbacV1().ClusterRoles().Get(context.TODO(), "cluster-autoscaler-dynamic-apps-deployments", metav1.GetOptions{})
	assert.Error(t, err)
}
