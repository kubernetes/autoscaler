...
// Scenario represents a benchmark test case, specifying the setup, verification, and optional custom configurations.
...
func setupScaleUp(nodes int) func(*integration.FakeSet) error {
	return func(clusterFakes *integration.FakeSet) error {
		nTemplate := test.BuildTestNode("n-template", nodeCPU, nodeMem)
		test.SetNodeReadyState(nTemplate, true, time.Now())
		...
		for i := range nodes * podsPerNode {
			...
			pod := test.BuildTestPod(podName, cpu, mem, test.MarkUnschedulable())
...
func setupScaleDown60Percent(nodesCount int) func(*integration.FakeSet) error {
	return func(clusterFakes *integration.FakeSet) error {
		...
		for i := range nodesCount * 40 {
			...
			pod := test.BuildTestPod(podName, cpu, mem)
