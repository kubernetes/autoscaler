...
func TestFilterOutSchedulableKarpenterEnabled(t *testing.T) {
	node := buildReadyTestNode("node", 2000, 100)
	schedulablePod := BuildTestPod("schedulable-pod", 500, 10)
	unschedulablePod := BuildTestPod("unschedulable-pod", 3000, 10)
	...
	unschedulablePods := []*apiv1.Pod{schedulablePod, unschedulablePod}
	processedPods, err := processor.Process(autoscalingCtx, unschedulablePods)

	assert.NoError(t, err)
	assert.Equal(t, []*apiv1.Pod{unschedulablePod}, processedPods, "Karpenter-aware filterOutSchedulable should filter out schedulable pod and keep truly unschedulable pod")
}
