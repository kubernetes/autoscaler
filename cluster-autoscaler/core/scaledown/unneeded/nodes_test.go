...
func TestNodeLoadFromExistingTaints(t *testing.T) {
	...
	// LoadFromExistingTaints doesn't actually removes deletionCandidate taints, so this test assumes that taints are valid
	// and this is why the test cases are limited.
	testCases := []struct {
		name                     string
		allNodes                 []*apiv1.Node
