...
func New(autoscalingCtx *ca_context.AutoscalingContext, processors *processors.AutoscalingProcessors, deleteOptions options.NodeDeleteOptions, drainabilityRules rules.Rules) *Planner {
	...
	var longestNodeScaleDownEvalTime *longestNodeScaleDownEvalTime
	if autoscalingCtx.AutoscalingOptions.LongestNodeScaleDownEvalTimeTrackerEnabled {
		longestNodeScaleDownEvalTime = newLongestNodeScaleDownEvalTime(time.Now())
	}
...
func (p *Planner) handleUnprocessedNodes(unprocessedNodeNames []string, currentTime time.Time, endedPrematurely *bool) {
	// if p.longestNodeScaleDownEvalTime is not set (flag is disabled) or endedPrematurely is already true (ndoes were already reported in this iteration) do not do anything
	if p.longestNodeScaleDownEvalTime == nil || *endedPrematurely {
		return
	}
	*endedPrematurely = true
	p.longestNodeScaleDownEvalTime.update(unprocessedNodeNames, currentTime)
}
