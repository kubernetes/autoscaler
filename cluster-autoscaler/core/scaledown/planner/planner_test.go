	...
	testCases := []testCase{
		{
			...
			unprocessedNodes:             [][]string{nil, {"n1", "n2"}, {"n2", "n3"}, nil, nil},
			wantLongestScaleDownEvalTime: []time.Duration{time.Duration(0), time.Duration(1 * time.Second), time.Duration(2 * time.Second), time.Duration(3 * time.Second), time.Duration(0)},
		},
		{
			name:                         "Test the functionality of longestNodeScaleDownT with not all nodes processed in the first iteration",
			unprocessedNodes:             [][]string{{"n1", "n2"}, {"n1", "n2"}, {"n2", "n3"}, nil, nil},
			wantLongestScaleDownEvalTime: []time.Duration{time.Duration(1 * time.Second), time.Duration(2 * time.Second), time.Duration(3 * time.Second), time.Duration(4 * time.Second), time.Duration(0)},
		},
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for i := 0; i < len(tc.unprocessedNodes); i++ {
				...
				assert.Equal(t, longestScaleDownEvalT.update(tc.unprocessedNodes[i], timestamp), tc.wantLongestScaleDownEvalTime[i])
				assert.Equal(t, len(longestScaleDownEvalT.nodeNamesWithTimeStamps), len(tc.unprocessedNodes[i]))
			}
		})
	}
}
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if !tc.isFlagEnabled {
				...
				assert.Equal(t, len(p.longestNodeScaleDownT.nodeNamesWithTimeStamps), tc.wantUnprocessedNodes)
			}
		})
	}
}
