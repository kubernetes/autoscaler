...
func runSimpleScaleUpTest(t *testing.T, config *ScaleUpTestConfig) *ScaleUpTestResult {
	...
	err = autoscalingContext.ClusterSnapshot.SetClusterState(nodes, kube_util.ScheduledPods(pods), drasnapshot.Snapshot{})
	assert.NoError(t, err)
...
func TestScaleUpUnhealthy(t *testing.T) {
	...
	autoscalingContext, err := NewScaleTestAutoscalingContext(options, &fake.Clientset{}, listers, provider, nil, nil)
	assert.NoError(t, err)
	err = autoscalingContext.ClusterSnapshot.SetClusterState(nodes, pods, drasnapshot.Snapshot{})
	assert.NoError(t, err)
...
func TestBinpackingLimiter(t *testing.T) {
	...
	autoscalingContext, err := NewScaleTestAutoscalingContext(options, &fake.Clientset{}, listers, provider, nil, nil)
	assert.NoError(t, err)
	err = autoscalingContext.ClusterSnapshot.SetClusterState(nodes, nil, drasnapshot.Snapshot{})
	assert.NoError(t, err)
	nodeInfos, err := nodeinfosprovider.NewDefaultTemplateNodeInfoProvider(nil, false).
		Process(&autoscalingContext, nodes, []*appsv1.DaemonSet{}, taints.TaintConfig{}, now)
	assert.NoError(t, err)
