...
func runSimpleScaleUpTest(t *testing.T, config *ScaleUpTestConfig) *ScaleUpTestResult {
	...
	orchestrator := New()
	orchestrator.Initialize(&context, processors, clusterState, newEstimator(&context), taints.TaintConfig{})
...
func TestComputeSimilarNodeGroups(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			suOrchestrator.Initialize(&ctx, &processors.AutoscalingProcessors{NodeGroupSetProcessor: nodeGroupSetProcessor}, clusterState, newEstimator(&ctx), taints.TaintConfig{})
