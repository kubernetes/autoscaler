...
func TestScaleupAsyncNodeGroupsEnabledWithNoUpcomingNodegroup(t *testing.T) {
	...
	processors.NodeGroupListProcessor = &MockAutoprovisioningNodeGroupListProcessor{T: t}
	processors.NodeGroupManager = &MockAutoprovisioningNodeGroupManager{T: t, ExtraGroups: 0}
	processors.AsyncNodeGroupStateChecker = &MockAsyncNodeGroupStateCheker{T: t, IsUpcomingNodeGroup: map[string]bool{}}
...
func TestScaleupAsyncNodeGroupsEnabledWithUpcomingNodegroup(t *testing.T) {
	createdGroups := make(chan string, 10)
	expandedGroups := make(chan string, 10)
	...
	provider.AddNodeGroup("T1", 0, 10, 0)
	...
	processors.NodeGroupListProcessor = &MockAutoprovisioningNodeGroupListProcessor{T: t}
	processors.NodeGroupManager = &MockAutoprovisioningNodeGroupManager{T: t, ExtraGroups: 0}
	processors.AsyncNodeGroupStateChecker = &MockAsyncNodeGroupStateCheker{T: t, IsUpcomingNodeGroup: map[string]bool{"autoprovisioned-T1": true}}
	...
	assert.NoError(t, err)
	assert.True(t, scaleUpStatus.WasSuccessful())
	assert.Equal(t, 0, len(createdGroups))
	assert.Equal(t, 1, len(expandedGroups))
	assert.Equal(t, "autoprovisioned-T1-2", utils.GetStringFromChan(expandedGroups))
}
