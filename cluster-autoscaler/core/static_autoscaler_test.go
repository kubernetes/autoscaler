...
func setupTestStaticAutoscalerInstanceCreationErrorsForZeroOrMaxScaling(t *testing.T, nodes []cloudprovider.Instance, keepPartiallyFailedZeroOrMaxScalingNodeGroups bool) (*StaticAutoscaler, *mockprovider.NodeGroup) {
	provider := &mockprovider.CloudProvider{}
	...
	nodeGroupAtomic.On("GetOptions", options.NodeGroupDefaults).Return(
		&config.NodeGroupAutoscalingOptions{
			...
			KeepPartiallyFailedZeroOrMaxScalingNodeGroups: keepPartiallyFailedZeroOrMaxScalingNodeGroups,
		}, nil)
...
func TestStaticAutoscalerInstanceCreationErrorsForZeroOrMaxScaling(t *testing.T) {
	// Case 1: zero or max scale-up should remove all nodes in case when only some them have creation errors and KeepPartiallyFailedNodeGroups IS NOT set
	...
	// Case 2: zero or max scale-up should not remove any nodes if only some them have creation errors and KeepPartiallyFailedZeroOrMaxScalingNodeGroups IS set
	autoscaler, nodeGroupAtomic = setupTestStaticAutoscalerInstanceCreationErrorsForZeroOrMaxScaling(t, []cloudprovider.Instance{
		{
			Id: "D1",
			Status: &cloudprovider.InstanceStatus{
	...
	// Case 3: zero or max scale-up should remove all nodes if all of them fail, even if KeepPartiallyFailedZeroOrMaxScalingNodeGroups IS set
	autoscaler, nodeGroupAtomic = setupTestStaticAutoscalerInstanceCreationErrorsForZeroOrMaxScaling(t, []cloudprovider.Instance{
		{
			Id: "D1",
			Status: &cloudprovider.InstanceStatus{
...
func setupTestRemoveOldUnregisteredNodesAtomic(t *testing.T, now time.Time, keepPartiallyFailedZeroOrMaxScalingNodeGroups bool) (*clusterstate.ClusterStateRegistry, *context.AutoscalingContext, *clusterstate_utils.LogEventRecorder, chan string) {
	deletedNodes := make(chan string, 10)
...
func TestRemoveOldUnregisteredNodesAtomic(t *testing.T) {
	// Case 1: KeepPartiallyFailedZeroOrMaxScalingNodeGroups is NOT set
	now := time.Now()
	...
	// Case 2: KeepPartiallKeepPartiallyFailedZeroOrMaxScalingNodeGroupsyFailedNodeGroups IS set
	now = time.Now()
