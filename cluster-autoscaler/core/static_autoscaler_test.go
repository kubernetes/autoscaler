...
func setupAutoscaler(config *autoscalerSetupConfig) (*StaticAutoscaler, error) {
	...
	clusterState := clusterstate.NewClusterStateRegistry(provider, config.clusterStateConfig, autoscalingCtx.LogRecorder, NewBackoff(), processors.NodeGroupConfigProcessor, processors.AsyncNodeGroupStateChecker, scaleupfailures.NewScaleUpFailuresRegistry())
...
func TestStaticAutoscalerRunOnce(t *testing.T) {
	...
	clusterState := clusterstate.NewClusterStateRegistry(provider, clusterStateConfig, autoscalingCtx.LogRecorder, NewBackoff(), nodegroupconfig.NewDefaultNodeGroupConfigProcessor(options.NodeGroupDefaults), processors.AsyncNodeGroupStateChecker, scaleupfailures.NewScaleUpFailuresRegistry())
...
func TestStaticAutoscalerInstanceCreationErrors(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("forceDeleteEnabled=%t,forceDeleteImplemented=%t", tc.forceDeleteEnabled, tc.forceDeleteImplemented), func(t *testing.T) {
			...
			clusterState := clusterstate.NewClusterStateRegistry(provider, clusterStateConfig, autoscalingCtx.LogRecorder, NewBackoff(), nodeGroupConfigProcessor, asyncNodeGroupStateChecker, scaleupfailures.NewScaleUpFailuresRegistry())
			autoscaler := &StaticAutoscaler{
				AutoscalingContext:    &autoscalingCtx,
				clusterStateRegistry:  clusterState,
				lastScaleUpTime:       time.Now(),
...
func TestRemoveFixNodeTargetSize(t *testing.T) {
	...
	clusterState := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{
		...
		OkTotalUnreadyCount:       1,
	}, fakeLogRecorder, NewBackoff(), nodegroupconfig.NewDefaultNodeGroupConfigProcessor(autoscalingCtx.AutoscalingOptions.NodeGroupDefaults), asyncnodegroups.NewDefaultAsyncNodeGroupStateChecker(), scaleupfailures.NewScaleUpFailuresRegistry())
...
func TestStaticAutoscalerWithNodeDeclaredFeatures(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			clusterState := clusterstate.NewClusterStateRegistry(provider, clusterStateConfig, autoscalingCtx.LogRecorder, NewBackoff(), processors.NodeGroupConfigProcessor, processors.AsyncNodeGroupStateChecker, scaleupfailures.NewScaleUpFailuresRegistry())
