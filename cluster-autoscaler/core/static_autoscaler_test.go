...
func TestStaticAutoscalerInstanceCreationErrorsForZeroOrMaxScaling(t *testing.T) {
	// Case 1: zero or max scaling should remove all nodes in case when only some them have creation errors and KeepPartiallyFailedNodeGroups IS NOT set
	...
	// Case 2: zero or max scaling should not remove all nodes if only some them have creation errors and KeepPartiallyFailedZeroOrMaxScalingNodeGroups IS set
	autoscaler, nodeGroupAtomic = setupTestStaticAutoscalerInstanceCreationErrorsForZeroOrMaxScaling(t, []cloudprovider.Instance{
		{
			Id: "D1",
			Status: &cloudprovider.InstanceStatus{
	...
	// Case 3: zero or max scaling should remove all nodes if all of them fail, even if KeepPartiallyFailedZeroOrMaxScalingNodeGroups IS set
	autoscaler, nodeGroupAtomic = setupTestStaticAutoscalerInstanceCreationErrorsForZeroOrMaxScaling(t, []cloudprovider.Instance{
		{
			Id: "D1",
			Status: &cloudprovider.InstanceStatus{
