...
func setupAutoscaler(config *autoscalerSetupConfig) (*StaticAutoscaler, error) {
	...
	clusterState := clusterstate.NewClusterStateRegistry(provider, config.clusterStateConfig, autoscalingCtx.LogRecorder, NewBackoff(), processors.NodeGroupConfigProcessor, processors.AsyncNodeGroupStateChecker, autoscalingCtx.MetricsRegistry)
...
func TestStaticAutoscalerRunOnce(t *testing.T) {
	...
	clusterState := clusterstate.NewClusterStateRegistry(provider, clusterStateConfig, autoscalingCtx.LogRecorder, NewBackoff(), nodegroupconfig.NewDefaultNodeGroupConfigProcessor(options.NodeGroupDefaults), processors.AsyncNodeGroupStateChecker, autoscalingCtx.MetricsRegistry)
...
func TestStaticAutoscalerInstanceCreationErrors(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("forceDeleteEnabled=%t,forceDeleteImplemented=%t", tc.forceDeleteEnabled, tc.forceDeleteImplemented), func(t *testing.T) {
			...
			clusterState := clusterstate.NewClusterStateRegistry(provider, clusterStateConfig, autoscalingCtx.LogRecorder, NewBackoff(), nodeGroupConfigProcessor, asyncNodeGroupStateChecker, autoscalingCtx.MetricsRegistry)
			autoscaler := &StaticAutoscaler{
				AutoscalingContext:    &autoscalingCtx,
				clusterStateRegistry:  clusterState,
				lastScaleUpTime:       time.Now(),
...
func TestRemoveFixNodeTargetSize(t *testing.T) {
	...
	clusterState := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{
		...
		OkTotalUnreadyCount:       1,
	}, fakeLogRecorder, NewBackoff(), nodegroupconfig.NewDefaultNodeGroupConfigProcessor(autoscalingCtx.AutoscalingOptions.NodeGroupDefaults), asyncnodegroups.NewDefaultAsyncNodeGroupStateChecker(), ca_metrics.NewCaMetricsWithRegistry(metrics.NewKubeRegistry()))
...
func TestRemoveOldUnregisteredNodes(t *testing.T) {
	...
	clusterState := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{
		MaxTotalUnreadyPercentage: 10,
		OkTotalUnreadyCount:       1,
	}, fakeLogRecorder, NewBackoff(), nodegroupconfig.NewDefaultNodeGroupConfigProcessor(autoscalingCtx.AutoscalingOptions.NodeGroupDefaults), asyncnodegroups.NewDefaultAsyncNodeGroupStateChecker(), autoscalingCtx.MetricsRegistry)
	err := clusterState.UpdateNodes([]*apiv1.Node{ng1_1}, nil, now.Add(-time.Hour))
	assert.NoError(t, err)
...
func TestStaticAutoscalerWithNodeDeclaredFeatures(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			clusterState := clusterstate.NewClusterStateRegistry(provider, clusterStateConfig, autoscalingCtx.LogRecorder, NewBackoff(), processors.NodeGroupConfigProcessor, processors.AsyncNodeGroupStateChecker, autoscalingCtx.MetricsRegistry)
