...
func setUpScaleDownActuator(autoscalingContext *ca_context.AutoscalingContext, autoscalingOptions config.AutoscalingOptions) {
	...
	autoscalingContext.ScaleDownActuator = actuation.NewActuator(autoscalingContext, nil, deletiontracker.NewNodeDeletionTracker(0*time.Second), deleteOptions, rules.Default(deleteOptions), processorstest.NewTestProcessors(autoscalingContext).NodeGroupConfigProcessor, nil)
}
...
func setupAutoscaler(config *autoscalerSetupConfig) (*StaticAutoscaler, error) {
	...
	autoscalingContext.ScaleDownActuator = actuation.NewActuator(&autoscalingContext, clusterState, nodeDeletionTracker, deleteOptions, drainabilityRules, processors.NodeGroupConfigProcessor, draProvider)
...
func TestStaticAutoscalerRunOnceWithUnselectedNodeGroups(t *testing.T) {
	...
	for name, test := range tests {
		...
		t.Run(name, func(t *testing.T) {
			...
			autoscalingContext, err := NewScaleTestAutoscalingContext(autoscalingOptions, clientset, listerRegistry, provider, processorCallbacks, nil)
			assert.NoError(t, err)
			...
			clusterState := clusterstate.NewClusterStateRegistry(provider, clusterStateConfig, autoscalingContext.LogRecorder, NewBackoff(), nodegroupconfig.NewDefaultNodeGroupConfigProcessor(autoscalingOptions.NodeGroupDefaults), processors.AsyncNodeGroupStateChecker)

			// Setting the Actuator is necessary for testing any scale-down logic, it shouldn't have anything to do in this test.
			sdActuator := actuation.NewActuator(&autoscalingContext, clusterState, deletiontracker.NewNodeDeletionTracker(0*time.Second), options.NodeDeleteOptions{}, nil, processors.NodeGroupConfigProcessor, nil)
			autoscalingContext.ScaleDownActuator = sdActuator
			...
			err = autoscaler.RunOnce(time.Now().Add(5 * time.Hour))
			assert.NoError(t, err)
			newNode, err := clientset.CoreV1().Nodes().Get(context.TODO(), test.node.Name, metav1.GetOptions{})
			assert.NoError(t, err)
...
func TestStaticAutoscalerUpcomingScaleDownCandidates(t *testing.T) {
	...
	// Setting the Actuator is necessary for testing any scale-down logic, it shouldn't have anything to do in this test.
	actuator := actuation.NewActuator(&autoscalingContext, csr, deletiontracker.NewNodeDeletionTracker(0*time.Second), options.NodeDeleteOptions{}, nil, processorstest.NewTestProcessors(&autoscalingContext).NodeGroupConfigProcessor, nil)
...
func newScaleDownPlannerAndActuator(autoscalingContext *ca_context.AutoscalingContext, p *ca_processors.AutoscalingProcessors, cs *clusterstate.ClusterStateRegistry, nodeDeletionTracker *deletiontracker.NodeDeletionTracker) (scaledown.Planner, scaledown.Actuator) {
	...
	actuator := actuation.NewActuator(autoscalingContext, cs, nodeDeletionTracker, deleteOptions, nil, p.NodeGroupConfigProcessor, nil)
