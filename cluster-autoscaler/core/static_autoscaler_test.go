...
func TestStaticAutoscalerRunOnceWithUnselectedNodeGroups(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			autoscaler := buildStaticAutoscaler(t, provider, allNodes, fakeClient)
...
func TestCleaningSoftTaintsInScaleDown(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			autoscaler := buildStaticAutoscaler(t, provider, test.testNodes, fakeClient)
...
func buildStaticAutoscaler(t *testing.T, provider cloudprovider.CloudProvider, allNodes []*apiv1.Node, fakeClient *fake.Clientset) *StaticAutoscaler {
	autoscalingOptions := config.AutoscalingOptions{
		NodeGroupDefaults: config.NodeGroupAutoscalingOptions{
			ScaleDownUnneededTime:         time.Minute,
			ScaleDownUnreadyTime:          time.Minute,
