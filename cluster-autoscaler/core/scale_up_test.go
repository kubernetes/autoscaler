...
func runSimpleScaleUpTest(t *testing.T, config *scaleTestConfig) *scaleTestResults {
	...
	nodeInfos, _ := utils.GetNodeInfosForGroups(nodes, nil, provider, listers, []*appsv1.DaemonSet{},
		context.PredicateChecker, nil, context.NodeReadinessGraceTime)
...
func TestScaleUpUnhealthy(t *testing.T) {
	...
	nodeInfos, _ := utils.GetNodeInfosForGroups(nodes, nil, provider, listers, []*appsv1.DaemonSet{}, context.PredicateChecker, nil, 0)
	clusterState := clusterstate.NewClusterStateRegistry(provider, clusterstate.ClusterStateRegistryConfig{}, context.LogRecorder, newBackoff())
	clusterState.UpdateNodes(nodes, nodeInfos, time.Now())
...
func TestScaleUpAutoprovisionedNodeGroup(t *testing.T) {
	...
	nodes := []*apiv1.Node{}
	nodeInfos, _ := utils.GetNodeInfosForGroups(nodes, nil, provider, context.ListerRegistry, []*appsv1.DaemonSet{}, context.PredicateChecker, nil, 0)
