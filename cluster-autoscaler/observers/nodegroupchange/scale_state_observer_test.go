...
func TestRegisterFailedScaleUpDirectCall(t *testing.T) {
	...
	mockMetricsObj.On("RegisterFailedScaleUp", metrics.FailedScaleUpReason("OUT_OF_RESOURCES"), "nvidia.com/gpu", "nvidia-tesla-k80", "custom-dra-driver").Return()
	...
	producer.RegisterFailedScaleUp(nodeGroup, 3, errorInfo, "custom-dra-driver", now)

	mockMetricsObj.AssertCalled(t, "RegisterFailedScaleUp", metrics.FailedScaleUpReason("OUT_OF_RESOURCES"), "nvidia.com/gpu", "nvidia-tesla-k80", "custom-dra-driver")
...
func TestRegisterFailedScaleUpCallViaObserversList(t *testing.T) {
	...
	// No template node info for ng2, meaning GPU extraction should fall back to empty strings.
	provider.SetMachineTemplates(map[string]*framework.NodeInfo{})

	// Setup Mock Metrics Observer
	mockMetricsObj := &mockMetrics{}
	mockMetricsObj.On("RegisterFailedScaleUp", metrics.FailedScaleUpReason("TIMEOUT"), "", "", "dra-driver").Return()
	...
	list.RegisterFailedScaleUp(nodeGroup, 5, errorInfo, "dra-driver", now)

	// Assertions
	mockMetricsObj.AssertCalled(t, "RegisterFailedScaleUp", metrics.FailedScaleUpReason("TIMEOUT"), "", "", "dra-driver")
