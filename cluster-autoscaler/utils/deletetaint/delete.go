...
const (
	// Taints used to make the node unschedulable.
...
// Convert taint key to short name for logging
...
func addTaint(node *apiv1.Node, client kube_client.Interface, taintKey string, effect apiv1.TaintEffect) error {
	...
	for {
		...
		klog.V(1).Infof("Successfully added %s on node %v", getKeyShortName(taintKey), node.Name)
		return nil
	}
}
...
// HasToBeDeletedTaint returns true if DeletionCandidate taint is applied on the node.
