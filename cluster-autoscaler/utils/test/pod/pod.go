...
func WithGPU(gpusCount int64) func(*apiv1.Pod) {
	...
	return func(pod *apiv1.Pod) {
		...
		for i := range pod.Spec.Containers {
			pod.Spec.Containers[i].Resources.Requests[test.ResourceNvidiaGPU] = *resource.NewQuantity(gpusCount, resource.DecimalSI)
			pod.Spec.Containers[i].Resources.Limits[test.ResourceNvidiaGPU] = *resource.NewQuantity(gpusCount, resource.DecimalSI)
		}
	}
}
...
func WithGPUToleration() func(*apiv1.Pod) {
	...
	return func(pod *apiv1.Pod) {
		pod.Spec.Tolerations = append(pod.Spec.Tolerations, apiv1.Toleration{Key: test.ResourceNvidiaGPU, Operator: apiv1.TolerationOpExists})
	}
}
...
func AsDaemonSetPod(uid string) func(*apiv1.Pod) {
	...
	return func(pod *apiv1.Pod) {
		pod.OwnerReferences = test.GenerateOwnerReferences("ds", "DaemonSet", "apps/v1", types.UID(uid))
	}
}
...
func AsReplicaSetPod(uid string, rsName string) func(*apiv1.Pod) {
	...
	return func(pod *apiv1.Pod) {
		pod.OwnerReferences = test.GenerateOwnerReferences(rsName, "ReplicaSet", "extensions/v1beta1", types.UID(uid))
	}
}
