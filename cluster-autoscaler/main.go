...
var (
	...
	bulkGceMigInstancesListingEnabled  = flag.Bool("bulk-mig-instances-listing-enabled", false, "Bulk GCE mig instances listing fetches GCE mig instances for all migs using one API and sort them in to migs. Instances of migs having instances in creating or deleting state are re-fetched using mig specific api call. Inconsistencies are handled by re-fetching using mig specific api calls")
