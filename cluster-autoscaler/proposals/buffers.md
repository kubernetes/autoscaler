...
type BufferSpec struct {
    ...
    //     the ReplicaBufferCapacity will be used
    //   - for autoscaler specifi node class the NodeClassBufferCapacity will be used
    //   - in case of no target object the user will be able to directly define needed capacity via pod definition
...
chunks will take into account the k8s Resource Quotas. Buffer controller will
take into account RecreationStrategy (including KeepMinimum so that the buffer
is adjusted to pods that are already running in the cluster).
...
This design work is not mature enough yet (no KEP proposed) so we will stick
with PodSpec for now and expand to new objects definition when they are
available.
...
### Why does NodeClassBufferCapacity not have relative size?

The ReplicaBufferCapacity has an option to set the size relative to the number
of pods in the scale subsource. But the meaning of defining NodeClassBufferCapacity
...
For full context this is how they would differ from the first milestone:

|                                | Buffer not dedicated for any workload (any workload matching the node can use it) - the only option in the first interation                                                                                             | Buffer dedicated for a workload (defined by pod selector) - possible to add later                                                                                                                                                                                                                                                                                                      |
|--------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
