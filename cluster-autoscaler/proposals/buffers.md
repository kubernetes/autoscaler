...
A `NodeAutoscalingBuffer` CRD (autoscaling.x-k8s.io) is added to represent spare capacity
...
it should be relatively easy to add needed fields to the ResourceBufferSpec.
...
the `spec.resources.perChunkResources`
...
```go
type NodeAutoscalingBuffer struct {
    metav1.TypeMeta
    metav1.ObjectMeta

    Spec   NodeAutoscalingBufferSpec
    Status NodeAutoscalingBufferStatus
}

type NodeAutoscalingBufferSpec struct {
    Capacity  NodeAutoscalingBufferCapacity

    // Initially only active stand by meaning scaling up the cluster will be available.
    // Not specified strategy will mean using buffer.x-k8s.io/active-capacity	
    // In the future the cloud providers will be able to offer their own buffering strategies.
    ProvisioningStrategy string
}

type NodeAutoscalingBufferCapacity struct {
    // One of the following must be set
    // +optional
    ControllerBased *ReplicaBufferCapacity
    // +optional
    Resources *ResourceBufferCapacity
    // +optional
    Pods *PodBufferCapacity
}

type ReplicaBufferCapacity struct {
    // An object implementing scale subresource and setting its label selector
    // field (for example deployment, job etc.)
...
type ResourceBufferCapacity struct {
    // Defines where the resource should be created.
    TargetRef *TypedLocalObjectReference
    // Upper limit of the total capacity that will be scaled for this buffer.
    // The total capacity will be split into smaller chunks that each fits onto 
    // nodes. The size of a chunk will be either decided for the user (using
    // some defaults and heuristics) or the user can specify per pod resources 
    // to define the shape of the buffer space. In the latter case the 
    // TotalResourceLimits will be the maximum size of the buffer and if the per
    // chunk resources are too big to fit the total buffer size the admission of
    // the buffer should be denied.
    TotalResourceLimits ResourceList
    ...
    PerChunkResources *ResourceList
...
### What is the responsibility split between the NodeAutoscalingBuffers controller and autoscaler?
...
mature enough yet (no KEP proposed) so we will stick with pod definition for now
and possibly expand to new objects definition when they are available.
...
### Why does ResourceBufferCapacity not have relative size?

The ReplicaBufferCapacity has an option to set the size relative to the number
of pods in the scale subresource. But the meaning of defining 
ResourceBufferCapacity compared to the current size is hard to define.
