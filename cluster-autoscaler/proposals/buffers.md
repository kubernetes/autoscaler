...
*   Buffer dedicated for a workload - make it possible to create a buffer that 
    would be dedicated for a workload and that can be "filled in" by it to limit
    over provisioning if the user knows the max capacity needed by their
    workload.
...
type ReplicaBufferCapacity struct {
	// An object implementing scale subresource (for example deployment, job etc.)
...
type PodSpecBufferCapacity struct {
    ReplicaNum int
    PodSpec    PodSpec
}

type NodeAutoscalingBufferStatus struct {
    PodSpecBufferCapacity   PodBufferCapacity
...
many and what spare capacity to deploy. Every time the autoscaler accomplished
capacity provisioning the autoscaler will also set `LastFullyProvisioned`. 
