...
type CapacityBufferSpec struct {
  // optional
  // oneof=Capacity
  ScaleSubresourceCapacity *ScaleSubresourceBufferCapacity

  // optional
  // oneof=Capacity
  PodCapacity *PodBufferCapacity
...
type ScaleSubresourceBufferCapacity struct {
  // An object implementing scale subresource and setting its label selector
  // field (for example deployment, job etc.)
  ScaleTargetRef TypedLocalObjectReference
  // If neither is set we will create as many chunks as fit in limits, if both are set we will take
  // max from these.
  // +optional
  Replicas *int
  // +optional
...
type CapacityBufferStatus struct {
  ResolvedBufferCapacity PodBufferCapacity
