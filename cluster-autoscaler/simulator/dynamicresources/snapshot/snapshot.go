...
// globalResourceSlicesIdentifier is a special key used in the resourceSlices patchSet
// to store ResourceSlices that apply to all nodes. The string itself is using a value
// which kubernetes nodes cannot possibly have to avoid having collisions.
const globalResourceSlicesIdentifier = "--GLOBAL--"
...
func NewSnapshot(
	...
	slices[globalResourceSlicesIdentifier] = globalSlices
...
func (s *Snapshot) AddClaims(newClaims []*resourceapi.ResourceClaim) error {
	for _, claim := range newClaims {
		_, found := s.resourceClaims.FindValue(GetClaimId(claim))
		if found {
...
// listResourceSlices retrieves all effective ResourceSlices (both node-local and global) from the snapshot.
