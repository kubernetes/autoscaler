...
func CloneTestSnapshot(snapshot *Snapshot) *Snapshot {
	...
	for i := 0; i < len(snapshot.deviceClasses.patches); i++ {
		...
		maps.Copy(clonedPatch.Modified, devicesPatch.Modified)
	...
	for i := 0; i < len(snapshot.resourceSlices.patches); i++ {
		...
		maps.Copy(clonedPatch.Modified, slicesPatch.Modified)
...
// SnapshotComparer returns a cmp.Option that provides a custom comparer function
// for comparing two *Snapshot objects based on their underlying data maps.
// This function is intended for testing purposes only.
func SnapshotComparer() cmp.Option {
