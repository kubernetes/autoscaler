...
| `node-readiness-grace-time` | A grace period CA gives a Kubernetes node to be considered as ready before basing node-group node templates from it. Useful for when node labels or allocatable resources are modified at runtime. Set to 0 to disable such grace time. | 5 minutes
| `node-cache-expiration-time` | The time CA waits before expiring stale Kubernetes nodes from its node-group node template cache. Post expiration, CA will retrieve node template information from the cloud provider instead of the previously cached Kubernetes node. Useful for node groups that scale down to zero, and have their labels or allocatable resources modified during termination. Setting this to -1 will cause cache entries to never expire.| 10 minutes
