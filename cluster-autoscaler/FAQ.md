...
When using this class, Cluster Autoscaler performs following actions:
  ...
  * __Condition Updates__:
  ...
  It should be done with caution by specifying `--check-capacity-provisioning-class-processor-instance=<name>` flag.
...
| `check-capacity-provisioning-class-processor-instance` | Name of the processor instance. Only ProvisioningRequests that define this name in their parameters with the key "processorInstance" will be processed by this CA instance. It only refers to check capacity ProvisioningRequests. Not recommended: Until CA 1.35, ProvisioningRequests with this name as prefix in their class will be also processed. |  |
