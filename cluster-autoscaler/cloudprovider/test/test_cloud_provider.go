...
// IsNodeCandidateForDeletion is a function called to determine if a cloud provider considers
// a node a good candidate for deletion.
type IsNodeCandidateForDeletion func(*apiv1.Node) (bool, error)
...
type TestCloudProvider struct {
	...
	isNodeCandidateForDeletion func(*apiv1.Node) (bool, error)
	machineTypes               []string
...
// WithIsNodeCandidateForDeletion adds an IsNodeCandidateForDeletion handler to provider.
func (b *TestCloudProviderBuilder) WithIsNodeCandidateForDeletion(isNodeCandidateForDeletion IsNodeCandidateForDeletion) *TestCloudProviderBuilder {
	b.builders = append(b.builders, func(p *TestCloudProvider) {
		p.isNodeCandidateForDeletion = isNodeCandidateForDeletion
	})
	return b
}
...
func (tcp *TestCloudProvider) IsNodeCandidateForDeletion(node *apiv1.Node) (bool, error) {
	if tcp.isNodeCandidateForDeletion == nil {
		return true, cloudprovider.ErrNotImplemented
	}

	return tcp.isNodeCandidateForDeletion(node)
}
