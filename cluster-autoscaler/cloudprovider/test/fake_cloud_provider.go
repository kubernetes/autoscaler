...
// WithKubeClient links the CloudProvider to a Kubernetes fake for automatic syncing.
func WithKubeClient(k8s *fakek8s.Kubernetes) CloudProviderOption {
...
func NewCloudProvider(opts ...CloudProviderOption) *CloudProvider {
	cp := &CloudProvider{
...
func (c *CloudProvider) GetResourceLimiter() (*cloudprovider.ResourceLimiter, error) {
	c.Lock()
	defer c.Unlock()
	// NewResourceLimiter deep copies these, so it's thread-safe for CA to use.
...
func (n *NodeGroup) DeleteNodes(nodes []*apiv1.Node) error {
	...
	// Update the provider's map so the node is "gone" from the cloud
	n.provider.Lock()
...
func (n *NodeGroup) Nodes() ([]cloudprovider.Instance, error) {
	...
	for i := 0; i < n.targetSize; i++ {
		instances = append(instances, cloudprovider.Instance{
			Id: fmt.Sprintf("%s-node-%d", n.id, i),
...
// Autoprovisioned returns true if the node group was created by the Autoprovisioner.
