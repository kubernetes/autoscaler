...
func (openstack *OpenStackCloudProvider) Pricing() (cloudprovider.PricingModel, errors.AutoscalerError) {
...
// OpenStackRef contains reference to some entity in OpenStack world.
...
func (ref OpenStackRef) String() string {
	return fmt.Sprintf("%s/%s/%s/%s", ref.Project, ref.RootResource, ref.Resource, ref.Name)
}

// OpenStackRefFromProviderId creates InstanceConfig object
// from provider id which must be in format:
// <project-id>/<root_resource_id>/<resource_id>/<name>
func OpenStackRefFromProviderId(id string) (*OpenStackRef, error) {
	splitted := strings.Split(id, "/")
	if len(splitted) != 4 {
		return nil, fmt.Errorf("Wrong id: expected format <project-id>/<root_resource_id>/<resource_id>/<name>, got %v", id)
	}
	return &OpenStackRef{
		Project:        splitted[0],
...
func GenerateASGUrl (ref OpenStackRef) string {
    return fmt.Sprintf("%s/%s/%s/%s", ref.Project, ref.RootResource, ref.Resource, ref.Name)
}
...
func (asg *openstackASG) Nodes() ([]cloudprovider.Instance, error) {
	instanceNames, err := asg.openstackManager.GetASGNodes(asg)
	if err != nil {
		return nil, err
	}
...
// TemplateNodeInfo returns a node template for this asg.
