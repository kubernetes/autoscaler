...
func (c *cachingMigInfoProvider) updateMigInstancesCache(migToInstances map[GceRef][]GceInstance) error {
	...
	for _, mig := range c.migLister.GetMigs() {
		migRef := mig.GceRef()
		// mig instances are re-fetched if there is an inconsistency between number
		// of instances according to the mig state and the instances found from instance.List api
