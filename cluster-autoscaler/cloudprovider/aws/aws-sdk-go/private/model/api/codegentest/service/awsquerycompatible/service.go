...
//
//	// Create a AwsQueryCompatible client from just a session.
//	svc := awsquerycompatible.New(mySession)
//
//	// Create a AwsQueryCompatible client with additional configuration
//	svc := awsquerycompatible.New(mySession, aws.NewConfig().WithRegion("us-west-2"))
...
func newClient(cfg aws.Config, handlers request.Handlers, partitionID, endpoint, signingRegion, signingName, resolvedRegion string) *AwsQueryCompatible {
	svc := &AwsQueryCompatible{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a AwsQueryCompatible operation and runs any
// custom request initialization.
func (c *AwsQueryCompatible) newRequest(op *request.Operation, params, data interface{}) *request.Request {
