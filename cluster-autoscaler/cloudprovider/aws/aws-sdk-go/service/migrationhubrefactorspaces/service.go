...
// MigrationHubRefactorSpaces provides the API operation methods for making requests to
// AWS Migration Hub Refactor Spaces. See this package's package overview docs
// for details on the service.
//
// MigrationHubRefactorSpaces methods are safe to use concurrently. It is not safe to
...
// New creates a new instance of the MigrationHubRefactorSpaces client with a session.
...
func New(p client.ConfigProvider, cfgs ...*aws.Config) *MigrationHubRefactorSpaces {
	...
	if c.SigningNameDerived || len(c.SigningName) == 0 {
		c.SigningName = "refactor-spaces"
	}
	return newClient(*c.Config, c.Handlers, c.PartitionID, c.Endpoint, c.SigningRegion, c.SigningName, c.ResolvedRegion)
}

// newClient creates, initializes and returns a new service client instance.
func newClient(cfg aws.Config, handlers request.Handlers, partitionID, endpoint, signingRegion, signingName, resolvedRegion string) *MigrationHubRefactorSpaces {
	svc := &MigrationHubRefactorSpaces{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a MigrationHubRefactorSpaces operation and runs any
// custom request initialization.
func (c *MigrationHubRefactorSpaces) newRequest(op *request.Operation, params, data interface{}) *request.Request {
