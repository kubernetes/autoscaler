...
// BackupGateway provides the API operation methods for making requests to
// AWS Backup Gateway. See this package's package overview docs
// for details on the service.
//
// BackupGateway methods are safe to use concurrently. It is not safe to
...
// New creates a new instance of the BackupGateway client with a session.
...
func New(p client.ConfigProvider, cfgs ...*aws.Config) *BackupGateway {
	...
	if c.SigningNameDerived || len(c.SigningName) == 0 {
		c.SigningName = "backup-gateway"
	}
	return newClient(*c.Config, c.Handlers, c.PartitionID, c.Endpoint, c.SigningRegion, c.SigningName, c.ResolvedRegion)
}

// newClient creates, initializes and returns a new service client instance.
func newClient(cfg aws.Config, handlers request.Handlers, partitionID, endpoint, signingRegion, signingName, resolvedRegion string) *BackupGateway {
	svc := &BackupGateway{
		Client: client.New(
			cfg,
			metadata.ClientInfo{
...
// newRequest creates a new request for a BackupGateway operation and runs any
// custom request initialization.
func (c *BackupGateway) newRequest(op *request.Operation, params, data interface{}) *request.Request {
