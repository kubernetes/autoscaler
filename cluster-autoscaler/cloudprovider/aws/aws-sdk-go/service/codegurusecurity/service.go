...
// CodeGuruSecurity provides the API operation methods for making requests to
// Amazon CodeGuru Security. See this package's package overview docs
// for details on the service.
//
// CodeGuruSecurity methods are safe to use concurrently. It is not safe to
...
func New(p client.ConfigProvider, cfgs ...*aws.Config) *CodeGuruSecurity {
	c := p.ClientConfig(EndpointsID, cfgs...)
	if c.SigningNameDerived || len(c.SigningName) == 0 {
		c.SigningName = "codeguru-security"
	}
	return newClient(*c.Config, c.Handlers, c.PartitionID, c.Endpoint, c.SigningRegion, c.SigningName, c.ResolvedRegion)
}

// newClient creates, initializes and returns a new service client instance.
func newClient(cfg aws.Config, handlers request.Handlers, partitionID, endpoint, signingRegion, signingName, resolvedRegion string) *CodeGuruSecurity {
...
// newRequest creates a new request for a CodeGuruSecurity operation and runs any
// custom request initialization.
func (c *CodeGuruSecurity) newRequest(op *request.Operation, params, data interface{}) *request.Request {
