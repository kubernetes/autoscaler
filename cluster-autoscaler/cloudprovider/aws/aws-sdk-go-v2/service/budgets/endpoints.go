...
import (
	...
	internalendpoints "k8s.io/autoscaler/cluster-autoscaler/cloudprovider/aws/aws-sdk-go-v2/service/budgets/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_BUDGETS")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "Budgets", cfg.ConfigSources)
	if found && err == nil {
	...
	if exprVal := params.Region; exprVal != nil {
		...
		if exprVal := awsrulesfn.GetPartition(_Region); exprVal != nil {
			...
			if _PartitionResult.Name == "aws" {
				if _UseFIPS == false {
					if _UseDualStack == false {
						uriString := "https://budgets.amazonaws.com"
			...
			if _PartitionResult.Name == "aws-cn" {
				if _UseFIPS == false {
					if _UseDualStack == false {
						uriString := "https://budgets.amazonaws.com.cn"
			...
			if _PartitionResult.Name == "aws-iso-e" {
				if _UseFIPS == false {
					if _UseDualStack == false {
						uriString := "https://budgets.global.cloud.adc-e.uk"
			...
			if _PartitionResult.Name == "aws-iso-f" {
				if _UseFIPS == false {
					if _UseDualStack == false {
						uriString := "https://budgets.global.csp.hci.ic.gov"
					...
					if true == _PartitionResult.SupportsFIPS {
						if true == _PartitionResult.SupportsDualStack {
							uriString := func() string {
								...
								out.WriteString("https://budgets-fips.")
