...
// usage examples for each of the [actions]and [data types] for the Proton service.
//
...
// more information, see the [Amazon Web Services Command Line Interface User Guide].
//
...
// Proton, developers need permissions to the service create, list, update and
// delete API operations and the service instance list and update API operations.
//
...
// than you intended.
//
// Idempotency ensures that an API request action completes no more than one time.
// With an idempotent request, if the original request action completes
...
// don't reuse the same client token for other API requests. If you don’t provide a
// client token for these APIs, a default client token is automatically provided by
// SDKs.
//
