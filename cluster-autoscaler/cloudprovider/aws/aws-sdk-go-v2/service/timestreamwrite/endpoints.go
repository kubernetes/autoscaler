...
import (
	...
	internalendpoints "k8s.io/autoscaler/cluster-autoscaler/cloudprovider/aws/aws-sdk-go-v2/service/timestreamwrite/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_TIMESTREAM_WRITE")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "Timestream Write", cfg.ConfigSources)
	if found && err == nil {
						...
						if true == _PartitionResult.SupportsDualStack {
							if "aws" == _PartitionResult.Name {
								uriString := func() string {
									...
									out.WriteString("https://timestream-ingest-fips.")
							...
							if "aws-us-gov" == _PartitionResult.Name {
								uriString := func() string {
									var out strings.Builder
									out.WriteString("https://timestream-ingest.")
									out.WriteString(_Region)
									out.WriteString(".api.aws")
									return out.String()
								}()
							...
							uriString := func() string {
								var out strings.Builder
								out.WriteString("https://ingest.timestream-fips.")
								out.WriteString(_Region)
								out.WriteString(".")
			...
			if _UseFIPS == true {
				if _PartitionResult.SupportsFIPS == true {
					if _PartitionResult.Name == "aws-us-gov" {
						uriString := func() string {
							var out strings.Builder
							out.WriteString("https://ingest.timestream.")
							out.WriteString(_Region)
