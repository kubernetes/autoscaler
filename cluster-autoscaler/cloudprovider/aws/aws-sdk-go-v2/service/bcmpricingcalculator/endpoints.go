...
import (
	...
	internalendpoints "k8s.io/autoscaler/cluster-autoscaler/cloudprovider/aws/aws-sdk-go-v2/service/bcmpricingcalculator/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_BCM_PRICING_CALCULATOR")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "BCM Pricing Calculator", cfg.ConfigSources)
	if found && err == nil {
		...
		if exprVal := awsrulesfn.GetPartition(_Region); exprVal != nil {
			...
			if _UseFIPS == true {
				uriString := func() string {
					...
					out.WriteString("https://bcm-pricing-calculator-fips.")
			...
			uriString := func() string {
				var out strings.Builder
				out.WriteString("https://bcm-pricing-calculator.")
				out.WriteString(_PartitionResult.ImplicitGlobalRegion)
				out.WriteString(".")
				out.WriteString(_PartitionResult.DualStackDnsSuffix)
				return out.String()
