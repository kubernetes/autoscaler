...
// EnvironmentReadyForDeploymentWaiterOptions are waiter options for
// EnvironmentReadyForDeploymentWaiter
type EnvironmentReadyForDeploymentWaiterOptions struct {
	...
	// EnvironmentReadyForDeploymentWaiter will use default minimum delay of 30
	// seconds. Note that MinDelay must resolve to a value lesser than or equal to the
	// MaxDelay.
	MinDelay time.Duration

	// MaxDelay is the maximum amount of time to delay between retries. If unset or
	// set to zero, EnvironmentReadyForDeploymentWaiter will use default max delay of
...
// EnvironmentReadyForDeploymentWaiter defines the waiters for
// EnvironmentReadyForDeployment
type EnvironmentReadyForDeploymentWaiter struct {
	client GetEnvironmentAPIClient

	options EnvironmentReadyForDeploymentWaiterOptions
}

// NewEnvironmentReadyForDeploymentWaiter constructs a
// EnvironmentReadyForDeploymentWaiter.
func NewEnvironmentReadyForDeploymentWaiter(client GetEnvironmentAPIClient, optFns ...func(*EnvironmentReadyForDeploymentWaiterOptions)) *EnvironmentReadyForDeploymentWaiter {
	options := EnvironmentReadyForDeploymentWaiterOptions{}
	options.MinDelay = 30 * time.Second
	options.MaxDelay = 120 * time.Second
	options.Retryable = environmentReadyForDeploymentStateRetryable

	for _, fn := range optFns {
		fn(&options)
	}
	return &EnvironmentReadyForDeploymentWaiter{
		client:  client,
		options: options,
	}
}

// Wait calls the waiter function for EnvironmentReadyForDeployment waiter. The
// maxWaitDur is the maximum wait duration the waiter will wait. The maxWaitDur is
// required and must be greater than zero.
func (w *EnvironmentReadyForDeploymentWaiter) Wait(ctx context.Context, params *GetEnvironmentInput, maxWaitDur time.Duration, optFns ...func(*EnvironmentReadyForDeploymentWaiterOptions)) error {
	_, err := w.WaitForOutput(ctx, params, maxWaitDur, optFns...)
	return err
}

// WaitForOutput calls the waiter function for EnvironmentReadyForDeployment
// waiter and returns the output of the successful operation. The maxWaitDur is the
// maximum wait duration the waiter will wait. The maxWaitDur is required and must
// be greater than zero.
func (w *EnvironmentReadyForDeploymentWaiter) WaitForOutput(ctx context.Context, params *GetEnvironmentInput, maxWaitDur time.Duration, optFns ...func(*EnvironmentReadyForDeploymentWaiterOptions)) (*GetEnvironmentOutput, error) {
	if maxWaitDur <= 0 {
	...
	return nil, fmt.Errorf("exceeded max wait time for EnvironmentReadyForDeployment waiter")
}

func environmentReadyForDeploymentStateRetryable(ctx context.Context, input *GetEnvironmentInput, output *GetEnvironmentOutput, err error) (bool, error) {

	if err == nil {
		v1 := output.State
		expectedValue := "ReadyForDeployment"
