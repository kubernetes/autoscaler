...
// days.
func (c *Client) ListAnalysisReports(ctx context.Context, params *ListAnalysisReportsInput, optFns ...func(*Options)) (*ListAnalysisReportsOutput, error) {
	if params == nil {
		params = &ListAnalysisReportsInput{}
	}

	result, metadata, err := c.invokeOperation(ctx, "ListAnalysisReports", params, optFns, c.addOperationListAnalysisReportsMiddlewares)
	if err != nil {
		return nil, err
	}

	out := result.(*ListAnalysisReportsOutput)
	out.ResultMetadata = metadata
	return out, nil
}

type ListAnalysisReportsInput struct {
...
// ListAnalysisReportsPaginatorOptions is the paginator options for
// ListAnalysisReports
type ListAnalysisReportsPaginatorOptions struct {
	// The maximum number of objects that you want Network Firewall to return for this
	// request. If more objects are available, in the response, Network Firewall
	// provides a NextToken value that you can use in a subsequent call to get the
	// next batch of objects.
...
// ListAnalysisReportsPaginator is a paginator for ListAnalysisReports
type ListAnalysisReportsPaginator struct {
	options   ListAnalysisReportsPaginatorOptions
	client    ListAnalysisReportsAPIClient
	params    *ListAnalysisReportsInput
	nextToken *string
	firstPage bool
}

// NewListAnalysisReportsPaginator returns a new ListAnalysisReportsPaginator
func NewListAnalysisReportsPaginator(client ListAnalysisReportsAPIClient, params *ListAnalysisReportsInput, optFns ...func(*ListAnalysisReportsPaginatorOptions)) *ListAnalysisReportsPaginator {
	if params == nil {
		params = &ListAnalysisReportsInput{}
	}
	...
	return &ListAnalysisReportsPaginator{
		options:   options,
		client:    client,
		params:    params,
		firstPage: true,
...
func (p *ListAnalysisReportsPaginator) HasMorePages() bool {
	return p.firstPage || (p.nextToken != nil && len(*p.nextToken) != 0)
}

// NextPage retrieves the next ListAnalysisReports page.
func (p *ListAnalysisReportsPaginator) NextPage(ctx context.Context, optFns ...func(*Options)) (*ListAnalysisReportsOutput, error) {
	if !p.HasMorePages() {
	...
	result, err := p.client.ListAnalysisReports(ctx, &params, optFns...)
	if err != nil {
		return nil, err
	}
	p.firstPage = false
...
// ListAnalysisReportsAPIClient is a client that implements the
// ListAnalysisReports operation.
type ListAnalysisReportsAPIClient interface {
	ListAnalysisReports(context.Context, *ListAnalysisReportsInput, ...func(*Options)) (*ListAnalysisReportsOutput, error)
}
