...
import (
	...
	internalendpoints "k8s.io/autoscaler/cluster-autoscaler/cloudprovider/aws/aws-sdk-go-v2/service/geomaps/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_GEO_MAPS")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "Geo Maps", cfg.ConfigSources)
	if found && err == nil {
				...
				if _UseFIPS == false {
					if _UseDualStack == false {
						uriString := func() string {
							...
							out.WriteString("https://maps.geo.")
			...
			if _PartitionResult.Name == "aws" {
				if _UseFIPS == true {
					if _UseDualStack == true {
						uriString := func() string {
							var out strings.Builder
							out.WriteString("https://maps.geo-fips.")
							out.WriteString(_Region)
							out.WriteString(".")
				...
				if _UseDualStack == true {
					if true == _PartitionResult.SupportsFIPS {
						if true == _PartitionResult.SupportsDualStack {
							uriString := func() string {
								var out strings.Builder
								out.WriteString("https://geo-maps-fips.")
								out.WriteString(_Region)
								out.WriteString(".")
								out.WriteString(_PartitionResult.DualStackDnsSuffix)
								return out.String()
			...
			if _UseFIPS == false {
				if _UseDualStack == true {
					if true == _PartitionResult.SupportsDualStack {
						uriString := func() string {
							var out strings.Builder
							out.WriteString("https://geo-maps.")
							out.WriteString(_Region)
							out.WriteString(".")
							out.WriteString(_PartitionResult.DualStackDnsSuffix)
							return out.String()
