...
func (c *Client) UpdateAIGuardrail(ctx context.Context, params *UpdateAIGuardrailInput, optFns ...func(*Options)) (*UpdateAIGuardrailOutput, error) {
	if params == nil {
		params = &UpdateAIGuardrailInput{}
	}

	result, metadata, err := c.invokeOperation(ctx, "UpdateAIGuardrail", params, optFns, c.addOperationUpdateAIGuardrailMiddlewares)
	if err != nil {
		return nil, err
	}

	out := result.(*UpdateAIGuardrailOutput)
...
func (c *Client) addOperationUpdateAIGuardrailMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	if err = addIdempotencyToken_opUpdateAIGuardrailMiddleware(stack, options); err != nil {
		return err
	}
...
type idempotencyToken_initializeOpUpdateAIGuardrail struct {
	tokenProvider IdempotencyTokenProvider
}

func (*idempotencyToken_initializeOpUpdateAIGuardrail) ID() string {
	return "OperationIdempotencyTokenAutoFill"
}

func (m *idempotencyToken_initializeOpUpdateAIGuardrail) HandleInitialize(ctx context.Context, in middleware.InitializeInput, next middleware.InitializeHandler) (
	...
	if !ok {
		return out, metadata, fmt.Errorf("expected middleware input to be of type *UpdateAIGuardrailInput ")
	}
