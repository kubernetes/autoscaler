...
func (c *Client) CreateCredentialLocker(ctx context.Context, params *CreateCredentialLockerInput, optFns ...func(*Options)) (*CreateCredentialLockerOutput, error) {
	...
	result, metadata, err := c.invokeOperation(ctx, "CreateCredentialLocker", params, optFns, c.addOperationCreateCredentialLockerMiddlewares)
	if err != nil {
		return nil, err
	}

	out := result.(*CreateCredentialLockerOutput)
...
func (c *Client) addOperationCreateCredentialLockerMiddlewares(stack *middleware.Stack, options Options) (err error) {
	...
	if err = addIdempotencyToken_opCreateCredentialLockerMiddleware(stack, options); err != nil {
		return err
	}
	if err = stack.Initialize.Add(newServiceMetadataMiddleware_opCreateCredentialLocker(options.Region), middleware.Before); err != nil {
		return err
	}
...
type idempotencyToken_initializeOpCreateCredentialLocker struct {
	tokenProvider IdempotencyTokenProvider
}

func (*idempotencyToken_initializeOpCreateCredentialLocker) ID() string {
	return "OperationIdempotencyTokenAutoFill"
}

func (m *idempotencyToken_initializeOpCreateCredentialLocker) HandleInitialize(ctx context.Context, in middleware.InitializeInput, next middleware.InitializeHandler) (
	...
	if !ok {
		return out, metadata, fmt.Errorf("expected middleware input to be of type *CreateCredentialLockerInput ")
	}
...
func addIdempotencyToken_opCreateCredentialLockerMiddleware(stack *middleware.Stack, cfg Options) error {
	return stack.Initialize.Add(&idempotencyToken_initializeOpCreateCredentialLocker{tokenProvider: cfg.IdempotencyTokenProvider}, middleware.Before)
}

func newServiceMetadataMiddleware_opCreateCredentialLocker(region string) *awsmiddleware.RegisterServiceMetadata {
