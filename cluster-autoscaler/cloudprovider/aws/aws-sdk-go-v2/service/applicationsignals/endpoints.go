...
import (
	...
	internalendpoints "k8s.io/autoscaler/cluster-autoscaler/cloudprovider/aws/aws-sdk-go-v2/service/applicationsignals/internal/endpoints"
...
func resolveBaseEndpoint(cfg aws.Config, o *Options) {
	...
	_, s := os.LookupEnv("AWS_ENDPOINT_URL_APPLICATION_SIGNALS")

	if g && !s {
		return
	}

	value, found, err := internalConfig.ResolveServiceBaseEndpoint(context.Background(), "Application Signals", cfg.ConfigSources)
	if found && err == nil {
		...
		if exprVal := awsrulesfn.GetPartition(_Region); exprVal != nil {
			...
			if _UseFIPS == true {
				uriString := func() string {
					...
					out.WriteString("https://application-signals-fips.")
			...
			uriString := func() string {
				var out strings.Builder
				out.WriteString("https://application-signals.")
				out.WriteString(_Region)
				out.WriteString(".")
				out.WriteString(_PartitionResult.DualStackDnsSuffix)
				return out.String()
