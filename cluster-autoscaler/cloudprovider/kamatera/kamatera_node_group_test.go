...
func TestNodeGroup_DeleteNodes(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := ng.DeleteNodes(context.TODO(), []*apiv1.Node{
			...
			err = ng.DeleteNodes(context.TODO(), []*apiv1.Node{{Spec: apiv1.NodeSpec{ProviderID: mockKamateraServerName()}}})
			assert.Error(t, err)
...
func TestNodeGroup_DeleteNodes_PoweroffBehavior(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := ng.DeleteNodes(context.TODO(), []*apiv1.Node{{Spec: apiv1.NodeSpec{ProviderID: serverProviderID}}})
			assert.NoError(t, err)
...
func TestNodeGroup_Nodes(t *testing.T) {
	...
	instancesList, err := ng.Nodes(context.TODO())
	assert.NoError(t, err)
...
func TestNodeGroup_Others(t *testing.T) {
	...
	assert.Equal(t, 1, ng.MinSize(context.TODO()))
	assert.Equal(t, 7, ng.MaxSize(context.TODO()))
	ts, err := ng.TargetSize(context.TODO())
	assert.NoError(t, err)
	assert.Equal(t, 3, ts)
	assert.Equal(t, "ng1", ng.Id())
	assert.Equal(t, "node group ID: ng1 (min:1 max:7)", ng.Debug(context.TODO()))
	...
	assert.Equal(t, true, ng.Exist(context.TODO()))
	assert.Equal(t, false, ng.Autoprovisioned(context.TODO()))
	_, err = ng.Create(context.TODO())
	assert.Error(t, err)
	assert.Equal(t, "Not implemented", err.Error())
