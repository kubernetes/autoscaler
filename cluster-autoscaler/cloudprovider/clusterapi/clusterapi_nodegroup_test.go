...
func TestNodeGroupDecreaseTargetSize(t *testing.T) {
	...
	test := func(t *testing.T, tc *testCase, testConfig *testConfig) {
		...
		case *machineSetScalableResource:
			...
			_, err := controller.dynamicclient.Resource(*controller.machineSetResource).Namespace(u.GetNamespace()).Update(u, metav1.UpdateOptions{})
			if err != nil {
		...
		case *machineDeploymentScalableResource:
			...
			_, err := controller.dynamicclient.Resource(*controller.machineDeploymentResource).Namespace(u.GetNamespace()).Update(u, metav1.UpdateOptions{})
			if err != nil {
				t.Fatalf("failed to updating machine: %v", err)
			}
		default:
...
func TestNodeGroupDeleteNodesTwice(t *testing.T) {
	addDeletionTimestampToMachine := func(t *testing.T, controller *machineController, node *corev1.Node) error {
		...
		if _, err := controller.dynamicclient.Resource(*controller.machineResource).Namespace(m.GetNamespace()).Update(newUnstructuredFromMachine(m), v1.UpdateOptions{}); err != nil {
			return err
		}
