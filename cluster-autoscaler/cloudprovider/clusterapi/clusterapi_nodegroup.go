...
func (ng *nodegroup) MinSize(ctx context.Context) int {
	return ng.scalableResource.MinSize(context.TODO())
}

func (ng *nodegroup) MaxSize(ctx context.Context) int {
	return ng.scalableResource.MaxSize(context.TODO())
}
...
func (ng *nodegroup) DecreaseTargetSize(ctx context.Context, delta int) error {
	...
	if size+delta < actualNodes {
		return fmt.Errorf("node group %s: attempt to delete existing nodes currentReplicas:%d delta:%d existingNodes: %d",
			ng.scalableResource.Name(context.TODO()), size, delta, actualNodes)
	}

	klog.V(4).Infof("%s: DecreaseTargetSize: scaling down: currentReplicas: %d, delta: %d, existingNodes: %d", ng.scalableResource.Name(context.TODO()), size, delta, len(nodes))
	return ng.scalableResource.SetSize(size + delta)
}
...
func (ng *nodegroup) TemplateNodeInfo(ctx context.Context) (*framework.NodeInfo, error) {
	...
	nodeName := fmt.Sprintf("%s-asg-%d", ng.scalableResource.Name(context.TODO()), rand.Int63())
	node := corev1.Node{
	...
	nodeInfo := framework.NewNodeInfo(&node, resourceSlices, framework.NewPodInfo(cloudprovider.BuildKubeProxy(ng.scalableResource.Name(context.TODO())), nil))
	if csiNode != nil {
...
func newNodeGroupFromScalableResource(controller *machineController, unstructuredScalableResource *unstructured.Unstructured) (*nodegroup, error) {
	...
	if scalableResource.MaxSize(context.TODO())-scalableResource.MinSize(context.TODO()) < 0 || scalableResource.MaxSize(context.TODO()) == 0 {
		klog.V(4).Infof("nodegroup %s has no scaling capacity, skipping", scalableResource.Name(context.TODO()))
		return nil, nil
	}
...
func buildGenericLabels(nodeName string) map[string]string {
	...
	m[corev1.LabelArchStable] = GetDefaultScaleFromZeroArchitecture().Name(context.TODO())
