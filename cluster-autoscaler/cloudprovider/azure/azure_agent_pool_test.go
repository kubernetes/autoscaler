...
func TestDeleteOutdatedDeployments(t *testing.T) {
	...
	for _, test := range testCases {
		...
		mockDeploymentClient := &DeploymentClientMock{
			FakeStore: test.deployments,
		}
		testAS.manager.azClient.deploymentClient = mockDeploymentClient

		err := testAS.deleteOutdatedDeployments()
		assert.Equal(t, test.expectedErr, err, test.desc)
		existedDeployments, _ := mockDeploymentClient.List(context.Background(), "")
...
func TestAgentPoolGetVMsFromCache(t *testing.T) {
	...
	mockVMClient.EXPECT().NewListPager(testAS.manager.config.ResourceGroup, gomock.Any()).Return(getFakeVMListPager(expectedVMs))
...
func TestGetVMIndexes(t *testing.T) {
	...
	mockVMClient.EXPECT().NewListPager(as.manager.config.ResourceGroup, gomock.Any()).Return(getFakeVMListPager(expectedVMs))
...
func TestAgentPoolIncreaseSize(t *testing.T) {
	...
	as.manager.azClient.virtualMachinesClient = mockVMClient
	expectedVMs := getExpectedVMs()
	mockVMClient.EXPECT().NewListPager(as.manager.config.ResourceGroup, gomock.Any()).DoAndReturn(func(string, interface{}) *runtime.Pager[armcompute.VirtualMachinesClientListResponse] {
...
func TestDeleteInstances(t *testing.T) {
	...
	mockSAClient := NewMockStorageAccountsClient(ctrl)
