...
func (agentPool *ContainerServiceAgentPool) TargetSize() (int, error) {
	...
	glog.V(5).Infof("Got new size %d for agent pool (%q)", count, agentPool.Name)
...
func (agentPool *ContainerServiceAgentPool) deleteNodesInternal(providerIDs []string) (deleted int, err error) {
	for _, providerID := range providerIDs {
		...
		if err != nil {
			glog.Errorf("Failed to delete virtual machine %q with error: %v", nodeName, err)
			return deleted, err
		}
...
func (agentPool *ContainerServiceAgentPool) DeleteNodes(nodes []*apiv1.Node) error {
	...
	if deleted != 0 {
		...
		if err != nil {
			glog.Errorf("Failed to set size for agent pool %q with error: %v", agentPool.Name, err)
		} else {
			glog.V(3).Infof("Size for agent pool %q has been updated to %d", agentPool.Name, targetSize)
		}
	}
	return deleteError
}
...
func (agentPool *ContainerServiceAgentPool) DecreaseTargetSize(delta int) error {
	...
	glog.V(5).Infof("DecreaseTargetSize get current size %d for agent pool %q", currentSize, agentPool.Name)
	...
	glog.V(5).Infof("DecreaseTargetSize get target size %d for agent pool %q", targetSize, agentPool.Name)
