...
// convertToPointerSlice converts a slice of InstanceViewStatus to a slice of pointers
func convertToPointerSlice(statuses []armcompute.InstanceViewStatus) []*armcompute.InstanceViewStatus {
	result := make([]*armcompute.InstanceViewStatus, len(statuses))
...
func newTestVMSSListForEdgeZones(capacity int64, name string) *armcompute.VirtualMachineScaleSet {
	return &armcompute.VirtualMachineScaleSet{
		...
		ExtendedLocation: &armcompute.ExtendedLocation{
			...
			Type: ptr.To(armcompute.ExtendedLocationTypes("EdgeZone")),
		},
	}
}
...
func newTestVMList(count int) []armcompute.VirtualMachine {
	...
	// For flexible orchestration, VMs need to reference their VMSS
	vmssID := "/subscriptions/test-subscription-id/resourceGroups/test-asg/providers/Microsoft.Compute/virtualMachineScaleSets/test-asg"
	for i := 0; i < count; i++ {
...
func TestScaleSetTargetSize(t *testing.T) {
	...
	spotScaleSet.Properties.VirtualMachineProfile = &armcompute.VirtualMachineScaleSetVMProfile{Priority: ptr.To(armcompute.VirtualMachinePriorityTypesSpot)}
	...
	for _, orchMode := range orchestrationModes {
		...
		mockVMSSClient.EXPECT().NewListPager(provider.azureManager.config.ResourceGroup, gomock.Any()).DoAndReturn(func(string, interface{}) *runtime.Pager[armcompute.VirtualMachineScaleSetsClientListResponse] {
			return getFakeVMSSListPager(expectedScaleSets)
		}).AnyTimes()
		provider.azureManager.azClient.virtualMachineScaleSetsClient = mockVMSSClient
		mockVMClient := NewMockVirtualMachinesClient(ctrl)
		mockVMClient.EXPECT().NewListPager(provider.azureManager.config.ResourceGroup, gomock.Any()).DoAndReturn(func(string, interface{}) *runtime.Pager[armcompute.VirtualMachinesClientListResponse] {
		...
		mockVMSSClient.EXPECT().Get(gomock.Any(), provider.azureManager.config.ResourceGroup, "spot-vmss", gomock.Any()).Return(armcompute.VirtualMachineScaleSetsClientGetResponse{VirtualMachineScaleSet: spotScaleSet}, nil).Times(1)
		provider.azureManager.azClient.virtualMachineScaleSetsClient = mockVMSSClient
		mockVMSSVMClient := NewMockVirtualMachineScaleSetVMsClient(ctrl)

		mockVMSSVMClient.EXPECT().NewListPager(provider.azureManager.config.ResourceGroup, "test-asg", gomock.Any()).DoAndReturn(func(string, string, interface{}) *runtime.Pager[armcompute.VirtualMachineScaleSetVMsClientListResponse] {
		...
		if orchMode == armcompute.OrchestrationModeUniform {
			...
			// TODO: Use appropriate method instead of ListVmssFlexVMsWithoutInstanceView
			mockVMClient.EXPECT().NewListPager(provider.azureManager.config.ResourceGroup, gomock.Any()).DoAndReturn(func(string, interface{}) *runtime.Pager[armcompute.VirtualMachinesClientListResponse] {
...
func TestScaleSetIncreaseSize(t *testing.T) {
	...
	for _, orchMode := range orchestrationModes {
		...
		mockVMSSClient.EXPECT().BeginCreateOrUpdate(gomock.Any(), provider.azureManager.config.ResourceGroup, gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(func(ctx context.Context, rg string, name string, vmss armcompute.VirtualMachineScaleSet, opts *armcompute.VirtualMachineScaleSetsClientBeginCreateOrUpdateOptions) (*runtime.Poller[armcompute.VirtualMachineScaleSetsClientCreateOrUpdateResponse], error) {
			return getFakeVMSSPoller()
		}).AnyTimes()
		provider.azureManager.azClient.virtualMachineScaleSetsClient = mockVMSSClient
		mockVMClient := NewMockVirtualMachinesClient(ctrl)
		mockVMClient.EXPECT().NewListPager(provider.azureManager.config.ResourceGroup, gomock.Any()).DoAndReturn(func(string, interface{}) *runtime.Pager[armcompute.VirtualMachinesClientListResponse] {
		...
		// TODO SDK v2: Mock BeginCreateOrUpdate if needed
	...
	for testName, testCase := range testCases {
		t.Run(testName, func(t *testing.T) {
			...
			if !testCase.isMissingInstanceView {
				expectedVMSSVMs[2].Properties.InstanceView = &armcompute.VirtualMachineScaleSetVMInstanceView{Statuses: convertToPointerSlice(testCase.statuses)}
			}
			...
			mockVMSSClient.EXPECT().BeginCreateOrUpdate(gomock.Any(), manager.config.ResourceGroup, gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(func(ctx context.Context, rg string, name string, vmss armcompute.VirtualMachineScaleSet, opts *armcompute.VirtualMachineScaleSetsClientBeginCreateOrUpdateOptions) (*runtime.Poller[armcompute.VirtualMachineScaleSetsClientCreateOrUpdateResponse], error) {
				return getFakeVMSSPoller()
			}).AnyTimes()
			manager.azClient.virtualMachineScaleSetsClient = mockVMSSClient
			mockVMSSVMClient := NewMockVirtualMachineScaleSetVMsClient(ctrl)
			mockVMSSVMClient.EXPECT().NewListPager(manager.config.ResourceGroup, "vmss-failed-upscale", gomock.Any()).DoAndReturn(func(string, string, interface{}) *runtime.Pager[armcompute.VirtualMachineScaleSetVMsClientListResponse] {
				return getFakeVMSSVMListPager(expectedVMSSVMs)
			}).AnyTimes()
...
func TestScaleSetIncreaseSizeOnVMSSUpdating(t *testing.T) {
	...
	expectedScaleSets := []armcompute.VirtualMachineScaleSet{
		{
			...
			Properties: &armcompute.VirtualMachineScaleSetProperties{
				ProvisioningState: ptr.To(string(armcompute.GalleryProvisioningStateUpdating)),
	...
	manager.azClient.virtualMachineScaleSetsClient = mockVMSSClient
	mockVMSSVMClient := NewMockVirtualMachineScaleSetVMsClient(ctrl)
	mockVMSSVMClient.EXPECT().NewListPager(manager.config.ResourceGroup, "vmss-updating",
		gomock.Any()).DoAndReturn(func(string, string, interface{}) *runtime.Pager[armcompute.VirtualMachineScaleSetVMsClientListResponse] {
		return getFakeVMSSVMListPager(expectedVMSSVMs)
	}).AnyTimes()
	manager.azClient.virtualMachineScaleSetVMsClient = mockVMSSVMClient
...
func TestScaleSetBelongs(t *testing.T) {
	...
	for _, orchMode := range orchestrationModes {
		...
		if orchMode == armcompute.OrchestrationModeUniform {
			// For uniform mode, VMs are not listed, only VMSS VMs
			mockVMClient.EXPECT().NewListPager(provider.azureManager.config.ResourceGroup, gomock.Any()).DoAndReturn(func(string, interface{}) *runtime.Pager[armcompute.VirtualMachinesClientListResponse] {
				return getFakeVMListPager([]armcompute.VirtualMachine{})
			}).AnyTimes()
			mockVMSSVMClient := NewMockVirtualMachineScaleSetVMsClient(ctrl)
			...
			provider.azureManager.config.EnableVmssFlexNodes = true
			// For flexible orchestration, VMs are listed with a filter
			mockVMClient.EXPECT().NewListPager(provider.azureManager.config.ResourceGroup, gomock.Any()).DoAndReturn(func(string, interface{}) *runtime.Pager[armcompute.VirtualMachinesClientListResponse] {
				return getFakeVMListPager(expectedVMs)
			}).AnyTimes()
		}
...
func TestScaleSetDeleteInstancesWithForceDeleteEnabled(t *testing.T) {
	...
	expectedVMSSVMs[0].Properties.ProvisioningState = ptr.To(string(armcompute.GalleryProvisioningStateDeleting))
	expectedVMSSVMs[2].Properties.ProvisioningState = ptr.To(string(armcompute.GalleryProvisioningStateDeleting))
	mockVMSSVMClient.EXPECT().NewListPager(manager.config.ResourceGroup, "test-asg", gomock.Any()).DoAndReturn(func(string, string, interface{}) *runtime.Pager[armcompute.VirtualMachineScaleSetVMsClientListResponse] {
		return getFakeVMSSVMListPager(expectedVMSSVMs)
	}).AnyTimes()
	err = manager.forceRefresh()
...
func getFakeVMSSPoller() (*runtime.Poller[armcompute.VirtualMachineScaleSetsClientCreateOrUpdateResponse], error) {
	handler := &fakePollerHandler[armcompute.VirtualMachineScaleSetsClientCreateOrUpdateResponse]{
		done:   true,
		result: armcompute.VirtualMachineScaleSetsClientCreateOrUpdateResponse{},
	}
...
func getFakeVMSSDeleteInstancesPoller() (*runtime.Poller[armcompute.VirtualMachineScaleSetsClientDeleteInstancesResponse], error) {
	handler := &fakePollerHandler[armcompute.VirtualMachineScaleSetsClientDeleteInstancesResponse]{
		done:   true,
		result: armcompute.VirtualMachineScaleSetsClientDeleteInstancesResponse{},
	}

	return runtime.NewPoller(
		&http.Response{StatusCode: http.StatusAccepted},
		runtime.Pipeline{},
		&runtime.NewPollerOptions[armcompute.VirtualMachineScaleSetsClientDeleteInstancesResponse]{
			Handler: handler,
		},
	)
}
