...
type NodeGroup struct {
	...
	conditions         []model.NodePoolCondition
	nodePoolSpec       *model.NodePoolSpec
	nodeInfoTemplate   *schedulerframework.NodeInfo
...
func getNodeIDsToDelete(ng *NodeGroup, _ []*apiv1.Node, currentSize int) ([]string, bool, error) {
...
func (ng *NodeGroup) Nodes() ([]cloudprovider.Instance, error) {
	nodes, err := ng.huaweiCloudManager.getNodes(ng.nodePoolId)
	if err != nil {
...
func (ng *NodeGroup) TemplateNodeInfo() (*schedulerframework.NodeInfo, error) {
	if ng.nodeInfoTemplate != nil {
		return ng.nodeInfoTemplate, nil
	}

	template, err := ng.getTemplate()
	if err != nil {
		return nil, err
	}
	node, err := ng.buildNodeFromTemplate(template)
	if err != nil {
		return nil, err
	}
	nodeInfo := schedulerframework.NewNodeInfo()
	nodeInfo.SetNode(node)
	ng.nodeInfoTemplate = nodeInfo
	return nodeInfo, nil
}
