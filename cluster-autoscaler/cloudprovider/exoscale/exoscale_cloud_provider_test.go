...
func (ts *cloudProviderTestSuite) TestExoscaleCloudProvider_Name() {
	ts.Require().Equal(ProviderName, ts.p.Name(context.TODO()))
}
...
func (ts *cloudProviderTestSuite) TestExoscaleCloudProvider_NodeGroupForNode_InstancePool() {
	...
	nodeGroup, err := ts.p.NodeGroupForNode(context.TODO(), &apiv1.Node{
		Spec: apiv1.NodeSpec{
...
func (ts *cloudProviderTestSuite) TestExoscaleCloudProvider_NodeGroups() {
	...
	instancePoolNodeGroup, err := ts.p.NodeGroupForNode(context.TODO(), &apiv1.Node{
		Spec: apiv1.NodeSpec{
	...
	sksNodepoolNodeGroup, err := ts.p.NodeGroupForNode(context.TODO(), &apiv1.Node{
		Spec: apiv1.NodeSpec{
	...
	ts.Require().Len(ts.p.NodeGroups(context.TODO()), 2)
}
