...
type asgCache struct {
	...
	instanceToAsg      map[RancherRef]cloudprovider.NodeGroup
	notInRegisteredAsg map[RancherRef]bool
	mutex              sync.Mutex
...
func newAsgCache() (*asgCache, error) {
	cache := &asgCache{
		...
		instanceToAsg:      make(map[RancherRef]cloudprovider.NodeGroup),
		notInRegisteredAsg: make(map[RancherRef]bool),
...
func (m *asgCache) invalidateUnownedInstanceCache() {
	...
	m.notInRegisteredAsg = make(map[RancherRef]bool)
}
...
func (m *asgCache) FindForInstance(instance *RancherRef) (cloudprovider.NodeGroup, error) {
	m.mutex.Lock()
	defer m.mutex.Unlock()
