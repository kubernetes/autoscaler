...
func TestNodeGroups(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if len(provider.NodeGroups(context.TODO())) != tc.expectedGroups {
				t.Fatalf("expected %d groups, got %d", tc.expectedGroups, len(provider.NodeGroups(context.TODO())))
			}
		})
	}
}
...
func TestNodeGroupForNode(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			ng, err := provider.NodeGroupForNode(context.TODO(), tc.node)
			if err != nil {
				t.Fatal(err)
			}
