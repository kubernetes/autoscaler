...
func (request ListPrivateEndpointAssociationsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListPrivateEndpointAssociationsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
	...
	if _, ok := mappingListPrivateEndpointAssociationsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
