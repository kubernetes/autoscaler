...
func (request ListComputeClustersRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListComputeClustersSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
	...
	if _, ok := mappingListComputeClustersSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
