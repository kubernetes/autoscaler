...
func (m CreateVtapDetails) ValidateEnumValue() (bool, error) {
	...
	if _, ok := mappingCreateVtapDetailsEncapsulationProtocolEnum[string(m.EncapsulationProtocol)]; !ok && m.EncapsulationProtocol != "" {
	...
	if _, ok := mappingCreateVtapDetailsSourceTypeEnum[string(m.SourceType)]; !ok && m.SourceType != "" {
	...
	if _, ok := mappingCreateVtapDetailsTrafficModeEnum[string(m.TrafficMode)]; !ok && m.TrafficMode != "" {
	...
	if _, ok := mappingCreateVtapDetailsTargetTypeEnum[string(m.TargetType)]; !ok && m.TargetType != "" {
