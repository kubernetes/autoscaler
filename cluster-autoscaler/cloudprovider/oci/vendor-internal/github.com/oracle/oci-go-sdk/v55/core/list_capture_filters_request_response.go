...
func (request ListCaptureFiltersRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListCaptureFiltersSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
	...
	if _, ok := mappingListCaptureFiltersSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingCaptureFilterLifecycleStateEnum[string(request.LifecycleState)]; !ok && request.LifecycleState != "" {
