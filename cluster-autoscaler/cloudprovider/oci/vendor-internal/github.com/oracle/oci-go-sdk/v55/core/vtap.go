...
func (m Vtap) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingVtapLifecycleStateEnum[string(m.LifecycleState)]; !ok && m.LifecycleState != "" {
	...
	if _, ok := mappingVtapLifecycleStateDetailsEnum[string(m.LifecycleStateDetails)]; !ok && m.LifecycleStateDetails != "" {
	...
	if _, ok := mappingVtapEncapsulationProtocolEnum[string(m.EncapsulationProtocol)]; !ok && m.EncapsulationProtocol != "" {
	...
	if _, ok := mappingVtapSourceTypeEnum[string(m.SourceType)]; !ok && m.SourceType != "" {
	...
	if _, ok := mappingVtapTrafficModeEnum[string(m.TrafficMode)]; !ok && m.TrafficMode != "" {
	...
	if _, ok := mappingVtapTargetTypeEnum[string(m.TargetType)]; !ok && m.TargetType != "" {
