...
func (request ListVtapsRequest) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingListVtapsSortByEnum[string(request.SortBy)]; !ok && request.SortBy != "" {
	...
	if _, ok := mappingListVtapsSortOrderEnum[string(request.SortOrder)]; !ok && request.SortOrder != "" {
	...
	if _, ok := mappingVtapLifecycleStateEnum[string(request.LifecycleState)]; !ok && request.LifecycleState != "" {
