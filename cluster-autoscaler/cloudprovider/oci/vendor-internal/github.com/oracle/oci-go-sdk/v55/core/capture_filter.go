...
func (m CaptureFilter) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingCaptureFilterLifecycleStateEnum[string(m.LifecycleState)]; !ok && m.LifecycleState != "" {
	...
	if _, ok := mappingCaptureFilterFilterTypeEnum[string(m.FilterType)]; !ok && m.FilterType != "" {
