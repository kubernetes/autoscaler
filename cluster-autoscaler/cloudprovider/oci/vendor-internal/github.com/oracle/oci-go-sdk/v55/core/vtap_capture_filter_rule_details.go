...
func (m VtapCaptureFilterRuleDetails) ValidateEnumValue() (bool, error) {
	errMessage := []string{}
	if _, ok := mappingVtapCaptureFilterRuleDetailsTrafficDirectionEnum[string(m.TrafficDirection)]; !ok && m.TrafficDirection != "" {
	...
	if _, ok := mappingVtapCaptureFilterRuleDetailsRuleActionEnum[string(m.RuleAction)]; !ok && m.RuleAction != "" {
