...
var (
	...
	longestUnneededNodeScaleDownEvalDurationSeconds = k8smetrics.NewGauge(
		&k8smetrics.GaugeOpts{
			Namespace: caNamespace,
			Name:      "longest_unneeded_node_scale_down_eval_duration_seconds",
			Help:      "Longest unneeded node evaluation time during ScaleDown.",
		},
	)
)
...
func RegisterAll(emitPerNodeGroupMetrics bool) {
	...
	legacyregistry.MustRegister(longestUnneededNodeScaleDownEvalDurationSeconds)
...
// ObserveLongestUnneededNodeScaleDownEvalDurationSeconds records the longest time during which node was not processed during ScaleDown.
...
func ObserveLongestUnneededNodeScaleDownEvalDurationSeconds(duration time.Duration) {
	longestUnneededNodeScaleDownEvalDurationSeconds.Set(duration.Seconds())
}
