...
type GCEOptions struct {
	...
	// BulkMigInstancesListingEnabled means that mig instances should be listed using a single api call and sort them into migs instead of using mig specific api call.
	// Instances of migs having instances in creating or deleting state are re-fetched using mig specific api call. Inconsistencies are handled by re-fetching using mig specific api calls
