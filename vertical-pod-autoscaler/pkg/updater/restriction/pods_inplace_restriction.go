...
func (ip *PodsInPlaceRestrictionImpl) CanUnboost(pod *apiv1.Pod, vpa *vpa_types.VerticalPodAutoscaler) bool {
	...
	ready := vpa_api_util.PodReady(pod)
	durationPassed := vpa_api_util.PodStartupBoostDurationPassed(pod, vpa)
	hasAnnotation := vpa_api_util.PodHasCPUBoostInProgress(pod)

	klog.V(2).InfoS("Checking if pod can be unboosted", "pod", klog.KObj(pod), "ready", ready, "durationPassed", durationPassed, "hasAnnotation", hasAnnotation)

	if !ready || !durationPassed || !hasAnnotation {
		return false
	}
	cr, present := ip.podToReplicaCreatorMap[getPodID(pod)]
	if present {
