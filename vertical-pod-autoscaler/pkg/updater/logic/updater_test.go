...
func TestRunOnce_MultipleVPAs(t *testing.T) {
	...
	assert.True(t, resource.MustParse("3").Equal(gotCPUReq),
		"Container's CPU request should be patched with the recommendation from `vpa-in-place` (3 CPU), got %s", gotCPUReq.String())
	...
	assert.True(t, resource.MustParse("300M").Equal(gotMemReq),
		"Container's memory request should be patched with the recommendation from `vpa-in-place` (300M), got %s", gotMemReq.String())
	...
	assert.True(t, resource.MustParse("3").Equal(gotCPULimit),
		"Container's CPU limit should be patched with the recommendation from `vpa-in-place` (3 CPU), got %s", gotCPULimit.String())
	...
	assert.True(t, resource.MustParse("300M").Equal(gotMemLimit),
		"Container's memory limit should be patched with the recommendation from `vpa-in-place` (300M), got %s", gotMemLimit.String())
}
