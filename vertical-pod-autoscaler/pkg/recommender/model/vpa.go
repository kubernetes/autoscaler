...
// hasMatchedPodsLocked checks if VPA has matched pods based on conditions.
// Caller must hold vpa.mutex (read or write lock).
func (vpa *Vpa) hasMatchedPodsLocked() bool {
...
// asStatusLocked returns VPA status representation.
// Caller must hold vpa.mutex (read or write lock).
func (vpa *Vpa) asStatusLocked() *vpa_types.VerticalPodAutoscalerStatus {
...
func (vpa *Vpa) HasMatchedPods() bool {
	vpa.mutex.RLock()
	defer vpa.mutex.RUnlock()
	return vpa.hasMatchedPodsLocked()
}
