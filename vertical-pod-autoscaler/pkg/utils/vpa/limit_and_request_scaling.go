...
// GetBoundaryRequestCPU returns the boundary (min/max) CPU request that can be specified with
// preserving the original limit to request ratio. Returns nil if no boundary exists
func GetBoundaryRequestCPU(originalRequest, originalLimit, boundaryLimit, defaultLimit *resource.Quantity) *resource.Quantity {
	if originalLimit == nil || originalLimit.Value() == 0 && defaultLimit != nil {
		originalLimit = defaultLimit
	}
...
// scaleQuantityProportionallyCPU returns value which has the same proportion to scaledQuantity as scaleResult has to scaleBase
...
// scaleQuantityProportionallyMem returns value which has the same proportion to scaledQuantity as scaleResult has to scaleBase
// It also returns a bool indicating if it had to cap result to MaxInt64 milliunits.
