...
type VerticalPodAutoscalerBuilder interface {
	...
	WithCPUStartupBoost(factor *int32, quantity *resource.Quantity, duration string) VerticalPodAutoscalerBuilder
...
func (b *verticalPodAutoscalerBuilder) WithCPUStartupBoost(factor *int32, quantity *resource.Quantity, duration string) VerticalPodAutoscalerBuilder {
	c := *b
	parsedDuration, _ := time.ParseDuration(duration)
	cpuStartupBoost := vpa_types.CPUStartupBoost{
		Duration: &meta.Duration{Duration: parsedDuration},
	}
	if factor != nil {
		cpuStartupBoost.Type = vpa_types.FactorStartupBoostType
		cpuStartupBoost.Factor = factor
	} else {
		cpuStartupBoost.Type = vpa_types.QuantityStartupBoostType
