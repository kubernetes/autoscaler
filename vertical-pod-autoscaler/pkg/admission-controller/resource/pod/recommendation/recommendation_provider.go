...
type Provider interface {
	GetContainersResourcesForPod(pod *corev1.Pod, vpa *vpa_types.VerticalPodAutoscaler) ([]vpa_api_util.ContainerResources, []vpa_api_util.ContainerResources, vpa_api_util.ContainerToAnnotationsMap, error)
}
...
func GetContainersResources(pod *corev1.Pod, vpaResourcePolicy *vpa_types.PodResourcePolicy, podRecommendation vpa_types.RecommendedPodResources, limitRange *corev1.LimitRangeItem,
	...
	// Only native sidecars are eligible for recommendations; plain init containers run once
	// and must never be patched, even if a recommendation happens to exist.
...
// getResourcesForPodContainers processes a list of containers and returns their recommended resources.
func getResourcesForPodContainers(containers []corev1.Container, pod *corev1.Pod, vpaResourcePolicy *vpa_types.PodResourcePolicy, podRecommendation vpa_types.RecommendedPodResources,
	limitRange *corev1.LimitRangeItem, addAll bool, annotations vpa_api_util.ContainerToAnnotationsMap) []vpa_api_util.ContainerResources {
	resources := make([]vpa_api_util.ContainerResources, len(containers))
	for i, container := range containers {
		resources[i] = getSingleContainerResources(container, pod, vpaResourcePolicy, podRecommendation, limitRange, addAll, annotations)
	}
	return resources
}

// getSingleContainerResources returns the recommended resources for a single container.
func getSingleContainerResources(container corev1.Container, pod *corev1.Pod, vpaResourcePolicy *vpa_types.PodResourcePolicy, podRecommendation vpa_types.RecommendedPodResources,
	limitRange *corev1.LimitRangeItem, addAll bool, annotations vpa_api_util.ContainerToAnnotationsMap) vpa_api_util.ContainerResources {
	...
	res := vpa_api_util.ContainerResources{}
	...
	if recommendation == nil {
		...
		res.Requests = containerRequests
	} else {
		res.Requests = recommendation.Target
	}
	...
	if containerControlledValues == vpa_types.ContainerControlledValuesRequestsAndLimits {
		proportionalLimits, limitAnnotations := vpa_api_util.GetProportionalLimit(containerLimits, containerRequests, res.Requests, defaultLimit)
		if proportionalLimits != nil {
			res.Limits = proportionalLimits
	...
	if addAll {
		...
		if _, ok := res.Requests[corev1.ResourceCPU]; !ok && hasCpuRequest {
			res.Requests[corev1.ResourceCPU] = cpuRequest
		}
		memRequest, hasMemRequest := containerRequests[corev1.ResourceMemory]
		if _, ok := res.Requests[corev1.ResourceMemory]; !ok && hasMemRequest {
			res.Requests[corev1.ResourceMemory] = memRequest
		}
		cpuLimit, hasCpuLimit := containerLimits[corev1.ResourceCPU]
		if _, ok := res.Limits[corev1.ResourceCPU]; !ok && hasCpuLimit {
			res.Limits[corev1.ResourceCPU] = cpuLimit
		}
		memLimit, hasMemLimit := containerLimits[corev1.ResourceMemory]
		if _, ok := res.Limits[corev1.ResourceMemory]; !ok && hasMemLimit {
			res.Limits[corev1.ResourceMemory] = memLimit
		}
	}
	return res
}
...
func (p *recommendationProvider) GetContainersResourcesForPod(pod *corev1.Pod, vpa *vpa_types.VerticalPodAutoscaler) (initResources, resources []vpa_api_util.ContainerResources, annotations vpa_api_util.ContainerToAnnotationsMap, err error) {
	...
	initContainerResources, containerResources := GetContainersResources(pod, resourcePolicy, *recommendedPodResources, containerLimitRange, false, annotations)
	...
	return initContainerResources, containerResources, annotations, nil
}
