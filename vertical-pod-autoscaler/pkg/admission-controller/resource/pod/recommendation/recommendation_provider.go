...
type Provider interface {
	GetContainersResourcesForPod(pod *core.Pod, vpa *vpa_types.VerticalPodAutoscaler) ([]vpa_api_util.ContainerResources, *utils.Annotations, *vpa_api_util.ContainerResources, error)
}
...
func (p *recommendationProvider) GetContainersResourcesForPod(pod *core.Pod, vpa *vpa_types.VerticalPodAutoscaler) ([]vpa_api_util.ContainerResources, *utils.Annotations, *vpa_api_util.ContainerResources, error) {
	if vpa == nil || pod == nil {
	...
	containerResources := GetContainersResources(pod, resourcePolicy, *recommendedPodResources, containerLimitRange, false, annotations.Container)
...
func GetPodResources(pod *core.Pod,
	...
	if containerControlledValues == vpa_types.ContainerControlledValuesRequestsAndLimits {
		proportionalLimits, _ := vpa_api_util.GetProportionalLimit(podLimits, podRequests, resources.Requests, core.ResourceList{})
		if proportionalLimits != nil {
		...
		if podLimits != nil {
			...
			for _, rn := range []core.ResourceName{core.ResourceCPU, core.ResourceMemory} {
