...
func TestCalculatePatches_StartupBoost(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			vpa: test.VerticalPodAutoscaler().WithName("name").WithContainer("container1").WithCPUStartupBoost(&factor, nil, "10s").Get(),
			recommendResources: []vpa_api_util.ContainerResources{
				{
					Requests: core.ResourceList{
						cpu: resource.MustParse("100m"),
		...
		},
		{
			...
			vpa: test.VerticalPodAutoscaler().WithName("name").WithContainer("container1").WithCPUStartupBoost(nil, &quantity, "10s").Get(),
			recommendResources: []vpa_api_util.ContainerResources{
				{
					Requests: core.ResourceList{
						cpu: resource.MustParse("100m"),
			...
			expectPatches: []resource_admission.PatchRecord{
				GetAddAnnotationPatch(annotations.StartupCPUBoostAnnotation, "{\"requests\":{\"cpu\":\"100m\"},\"limits\":{}}"),
				addResourceRequestPatch(0, cpu, "500m"),
				addLimitsPatch(0),
				addResourceLimitPatch(0, cpu, "500m"),
				GetAddAnnotationPatch(ResourceUpdatesAnnotation, "Pod resources updated by name: container 0: cpu request, cpu limit"),
			},
		},
		{
		...
		},
		{
			...
			vpa: test.VerticalPodAutoscaler().WithName("name").WithContainer("container1").WithCPUStartupBoost(&invalidFactor, nil, "10s").Get(),
			recommendResources: []vpa_api_util.ContainerResources{
				{
					Requests: core.ResourceList{
						cpu: resource.MustParse("100m"),
		...
		},
		{
			...
			vpa: test.VerticalPodAutoscaler().WithName("name").WithContainer("container1").WithCPUStartupBoost(nil, &invalidQuantity, "10s").Get(),
			recommendResources: []vpa_api_util.ContainerResources{
				{
					Requests: core.ResourceList{
						cpu: resource.MustParse("500m"),
			...
			maxAllowedCpu:      "",
			featureGateEnabled: true,
			expectError:        fmt.Errorf("boost quantity 200m is less than container's request 500m"),
		},
		{
			...
			vpa: test.VerticalPodAutoscaler().WithName("name").WithContainer("container1").WithCPUStartupBoost(&factor3, nil, "10s").Get(),
			recommendResources: []vpa_api_util.ContainerResources{
				{
					Requests: core.ResourceList{
						cpu: resource.MustParse("1"),
		...
		},
		{
			...
			vpa: test.VerticalPodAutoscaler().WithName("name").WithContainer("container1").WithCPUStartupBoost(&factor, nil, "10s").WithScalingMode("container1", vpa_types.ContainerScalingModeOff).Get(),
			recommendResources: []vpa_api_util.ContainerResources{
				{
					Requests: core.ResourceList{
						cpu: resource.MustParse("1"),
