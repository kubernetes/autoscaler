...
func GetAddResourceRequirementValuePatch(i int, kind string, resource corev1.ResourceName, quantity resource.Quantity, containerType model.ContainerType) resource_admission.PatchRecord {
	return resource_admission.PatchRecord{
		Op:    "add",
		Path:  fmt.Sprintf("%s/%d/resources/%s/%s", containerType.GetPatchPath(), i, kind, escapeJSONPatchPath(string(resource))),
