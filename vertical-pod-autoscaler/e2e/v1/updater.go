...
var _ = UpdaterE2eDescribe("Updater", ginkgo.Label("FG:InPlaceOrRecreate"), func() {
	...
	ginkgo.BeforeEach(func() {
		checkFeatureGateTestsEnabled(f, features.InPlaceOrRecreate, false, true)
	})
...
var _ = UpdaterE2eDescribe("Updater", ginkgo.Label("FG:CPUStartupBoost"), func() {
	f := framework.NewDefaultFramework("vertical-pod-autoscaling")
	f.NamespacePodSecurityEnforceLevel = podsecurity.LevelBaseline

	ginkgo.BeforeEach(func() {
		checkFeatureGateTestsEnabled(f, features.CPUStartupBoost, false, true)
	})

	ginkgo.It("Unboost pods when they become Ready", func() {
		const statusUpdateInterval = 10 * time.Second
...
func setupPodsForCPUBoost(f *framework.Framework, hamsterCPU, hamsterMemory string) *apiv1.PodList {
	...
	boostedCPU := "200m"
	setupHamsterController(f, controller.Kind, boostedCPU, hamsterMemory, defaultHamsterReplicas)
	...
	vpaCRD := test.VerticalPodAutoscaler().
		...
		WithCPUStartupBoost(&factor, nil, "1s").
