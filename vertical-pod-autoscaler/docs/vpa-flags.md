...
| --logtostderr | true | --logtostderr                            log to standard error instead of files |
| --min-tls-version | "tls1_2" |                 The minimum TLS version to accept.  Must be set to either tls1_2 |
| --one-output |  | --one-output                             If true, only write logs to their native severity level (vs also writing to each lower severity level; no effect when -logtostderr=true) |
| --port | 8000 |                               The port to listen on. |
| --profiling |  | --profiling                              Is debug/pprof endpoint enabled |
| --register-by-url |  | --register-by-url                        If set to true, admission webhook will be registered by URL (webhookAddress:webhookPort) instead of by service name |
| --register-webhook | true | --register-webhook                       If set to true, admission webhook object will be created on start up to register with the API server. |
| --reload-cert |  | --reload-cert                            If set to true, reload leaf certificate. |
| --skip-headers |  | --skip-headers                           If true, avoid header prefixes in the log messages |
| --skip-log-headers |  | --skip-log-headers                       If true, avoid headers when opening log files (no effect when -logtostderr=true) |
...
| --webhook-failure-policy-fail |  | --webhook-failure-policy-fail            If set to true, will configure the admission webhook failurePolicy to "Fail". Use with caution. |
...
| --cpu-integer-post-processor-enabled |  | --cpu-integer-post-processor-enabled                   Enable the cpu-integer recommendation post processor. The post processor will round up CPU recommendations to a whole CPU for pods which were opted in by setting an appropriate label on VPA object (experimental) |
...
| --humanize-memory |  | --humanize-memory                                      Convert memory values in recommendations to the highest appropriate SI unit with up to 2 decimal places for better readability. |
| --ignored-vpa-object-namespaces |  |                 Comma separated list of namespaces to ignore when searching for VPA objects. Empty means no namespaces will be ignored. |
| --kube-api-burst | 10 |                                 QPS burst limit when making requests to Kubernetes apiserver |
| --kube-api-qps | 5 |                                   QPS limit when making requests to Kubernetes apiserver |
| --kubeconfig |  |                                    Path to a kubeconfig. Only required if out-of-cluster. |
| --leader-elect |  | --leader-elect                                         Start a leader election client and gain leadership before executing the main loop. Enable this when running replicated components for high availability. |
...
| --memory-saver |  | --memory-saver                                         If true, only track pods which have an associated VPA |
...
| --use-external-metrics |  | --use-external-metrics                                 ALPHA.  Use an external metrics provider instead of metrics_server. |
...
| --use-admission-controller-status | true | --use-admission-controller-status                                 If true, updater will only evict pods when admission controller status is valid. |
| --v, | 4 | Level                                                         set the log level verbosity |
| --vmodule |  |                                              comma-separated list of pattern=N settings for file-filtered logging |
| --vpa-object-namespace |  |                                     Namespace to search for VPA objects. Empty means all namespaces will be used. |
