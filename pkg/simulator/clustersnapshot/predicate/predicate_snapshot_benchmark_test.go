	...
	for snapshotName, snapshotFactory := range snapshots {
		for _, tc := range testCases {
			...
			b.Run(fmt.Sprintf("%s: AddNodeInfo() %d", snapshotName, tc), func(b *testing.B) {
				for i := 0; i < b.N; i++ {
					b.StopTimer()
					assert.NoError(b, clusterSnapshot.SetClusterState(context.TODO(), nil, nil, nil, nil))
					b.StartTimer()
...
func BenchmarkListNodeInfos(b *testing.B) {
	...
	for snapshotName, snapshotFactory := range snapshots {
		for _, tc := range testCases {
			...
			clusterSnapshot, err := snapshotFactory()
			assert.NoError(b, err)
			err = clusterSnapshot.SetClusterState(context.TODO(), nodes, nil, nil, nil)
			if err != nil {
				assert.NoError(b, err)
			}
			b.ResetTimer()
	...
	for snapshotName, snapshotFactory := range snapshots {
		for _, ntc := range nodeTestCases {
			...
			for _, ptc := range podTestCases {
				...
				clusterSnapshot, err := snapshotFactory()
				assert.NoError(b, err)
				err = clusterSnapshot.SetClusterState(context.TODO(), nodes, pods, nil, nil)
				assert.NoError(b, err)
