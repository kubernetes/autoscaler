...
func TestMarkNodes(t *testing.T) {
	...
	_, err := MarkToBeDeleted(context.Background(), node, fakeClient, false)
...
func TestSoftMarkNodes(t *testing.T) {
	...
	node := BuildTestNode("node", 1000, 1000)
	fakeClient := buildFakeClientWithConflicts(t, node)
	_, err := MarkDeletionCandidate(context.Background(), node, fakeClient)
	assert.NoError(t, err)
...
func TestCheckNodes(t *testing.T) {
	...
	addTaintsToSpec(context.Background(), node, taints, false)
	fakeClient := buildFakeClientWithConflicts(t, node)
...
func TestCleanNodes(t *testing.T) {
	...
	updatedNode, err := CleanToBeDeleted(context.Background(), node, fakeClient, false)
...
func TestCleanNodesWithCordon(t *testing.T) {
	...
	addTaintsToSpec(context.Background(), node, taints, true)
	fakeClient := buildFakeClientWithConflicts(t, node)
	...
	updatedNode, err := CleanToBeDeleted(context.Background(), node, fakeClient, true)
	cleaned := !slices.Equal(updatedNode.Spec.Taints, node.Spec.Taints)
	assert.True(t, cleaned)
	assert.NoError(t, err)
...
func TestSoftCleanNodes(t *testing.T) {
	...
	updatedNode, err := CleanDeletionCandidate(context.Background(), node, fakeClient)
	cleaned := !slices.Equal(updatedNode.Spec.Taints, node.Spec.Taints)
	assert.True(t, cleaned)
	assert.NoError(t, err)
...
func TestCleanAllToBeDeleted(t *testing.T) {
	...
	fakeRecorder := kube_util.CreateEventRecorder(context.Background(), fakeClient, false)

	assert.Equal(t, 1, len(getNode(t, fakeClient, "n2").Spec.Taints))

	CleanAllToBeDeleted(context.Background(), []*apiv1.Node{n1, n2}, fakeClient, fakeRecorder, false)
...
func TestCleanAllDeletionCandidates(t *testing.T) {
	...
	CleanStaleDeletionCandidates(context.Background(), []*apiv1.Node{n1, n2}, fakeClient, fakeRecorder, time.Duration(0))
...
func TestFilterOutNodesWithStartupTaints(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			allNodes, readyNodes := FilterOutNodesWithStartupTaints(context.Background(), taintConfig, nodes, nodes)
...
func TestSanitizeTaints(t *testing.T) {
	...
	newTaints := SanitizeTaints(context.Background(), node.Spec.Taints, taintConfig)
...
func TestAddTaints(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			updatedNode, err := AddTaints(context.Background(), n, fakeClient, newTaints, false)
			assert.NoError(t, err)
			apiNode := getNode(t, fakeClient, "node")
...
func TestCleanTaints(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			updatedNode, err := CleanTaints(context.Background(), n, fakeClient, tc.taintsToRemove, false)
...
func TestCleanStaleDeletionCandidates(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			CleanStaleDeletionCandidates(context.Background(),
				tc.allNodes,
				fakeClient,
				kube_util.CreateEventRecorder(context.Background(), fakeClient, false),
				tc.nodeDeletionCandidateTTL,
			)

			allNodes, err := fakeClient.CoreV1().Nodes().List(context.Background(), metav1.ListOptions{})
			assert.NoError(t, err)
