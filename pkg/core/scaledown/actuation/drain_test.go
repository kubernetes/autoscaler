...
import (
	...
	simulator_fake "sigs.k8s.io/cluster-autoscaler/pkg/simulator/fake"
...
func TestDaemonSetEvictionForEmptyNodes(t *testing.T) {
	...
	for _, scenario := range testScenarios {
		...
		t.Run(scenario.name, func(t *testing.T) {
			...
			drainConfig := SingleRuleDrainConfig(autoscalingCtx.MaxGracefulTerminationSec)
			evictor := Evictor{
...
func TestPodsToEvict(t *testing.T) {
	for tn, tc := range map[string]struct {
		...
		t.Run(tn, func(t *testing.T) {
			...
			gotDsPods, gotNonDsPods := podsToEvict(nodeInfo, autoscalingCtx.DaemonSetEvictionForOccupiedNodes)
