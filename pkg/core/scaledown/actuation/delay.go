...
func WaitForDelayDeletion(ctx context.Context, node *apiv1.Node, nodeLister kubernetes.NodeLister, timeout time.Duration) errors.AutoscalerError {
	...
	if timeout != 0 && hasDelayDeletionAnnotation(node) {
		logger.V(1).Info("Wait for removing annotations on node", "annotationPrefix", DelayDeletionAnnotationPrefix, "node", klog.KObj(node))
		err := wait.Poll(5*time.Second, timeout, func() (bool, error) {
			logger.V(5).Info("Waiting for removing annotations on node", "annotationPrefix", DelayDeletionAnnotationPrefix, "node", klog.KObj(node))
		...
		if err == wait.ErrWaitTimeout {
			logger.Info("Delay node deletion timed out for node, delay deletion annotation wasn't removed within timeout period, this might slow down scale down.", "node", klog.KObj(node), "timeout", timeout)
		} else {
			logger.V(2).Info("Annotation removed from node", "annotationPrefix", DelayDeletionAnnotationPrefix, "node", klog.KObj(node))
		}
	}
	return nil
}
