...
func TestSoftTaintUpdate(t *testing.T) {
	...
	errs := UpdateSoftDeletionTaints(context.Background(), &actx, nil, nodes)
	assert.Empty(t, errs)
	...
	nodes = getAllNodes(t, fakeClient)
	errs = UpdateSoftDeletionTaints(context.Background(), &actx, []*apiv1.Node{n1000}, []*apiv1.Node{n2000})
	assert.Empty(t, errs)
	...
	nodes = getAllNodes(t, fakeClient)
	errs = UpdateSoftDeletionTaints(context.Background(), &actx, nodes, nil)
	assert.Empty(t, errs)
