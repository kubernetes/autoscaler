...
func TestFilterOutSchedulable(t *testing.T) {
	...
	for tn, tc := range testCases {
		t.Run(tn, func(t *testing.T) {
			...
			unschedulablePods, err := processor.filterOutSchedulableByPacking(context.Background(), &ca_context.AutoscalingContext{}, tc.unschedulableCandidates, clusterSnapshot)
	...
	for snapshotName, snapshotFactory := range snapshots {
		for _, tc := range tests {
			b.Run(fmt.Sprintf("%s: %d nodes %d scheduled %d pending", snapshotName, tc.nodes, tc.scheduledPods, tc.pendingPods), func(b *testing.B) {
				...
				if err := clusterSnapshot.SetClusterState(context.Background(), nodes, scheduledPods, nil, nil); err != nil {
				...
				for i := 0; i < b.N; i++ {
					...
					if stillPending, err := processor.filterOutSchedulableByPacking(context.Background(), &ca_context.AutoscalingContext{}, pendingPods, clusterSnapshot); err != nil {
						assert.NoError(b, err)
...
func TestFilterOutSchedulable_GracefulDegradation(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			unschedulablePods, err := processor.filterOutSchedulableByPacking(context.Background(), autoscalingCtx, tc.unschedulableCandidates, clusterSnapshot)
