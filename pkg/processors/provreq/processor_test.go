	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, condition := range test.conditions {
				conditions.AddOrUpdateCondition(context.Background(), test.provReq, condition, metav1.ConditionTrue, "", "", metav1.Now())
			}
...
func TestBookCapacityConsumed(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			conditions.AddOrUpdateCondition(context.Background(), provReq, v1.Provisioned, metav1.ConditionTrue, "", "", metav1.Now())
			...
			updated, err := client.ProvisioningRequest(context.Background(), "ns", "pr")
			if err != nil {
