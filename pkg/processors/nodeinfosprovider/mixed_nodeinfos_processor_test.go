...
func TestMixedNodeInfosProvider(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err := snapshot.SetClusterState(context.Background(), allNodes, nil, nil, nil)
			...
			nodeInfos, err := processor.Process(context.Background(), ctx, allNodes, nil, taints.TaintConfig{}, now)
