...
func TestWriteStatusConfigMap(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			result, err := WriteStatusConfigMap(context.Background(), ti.client, ti.namespace, clusterAutoscalerStatus, nil, "my-cool-configmap", currentTime)
...
func TestWriteStatusConfigMapMarshal(t *testing.T) {
	...
	result, err := WriteStatusConfigMap(context.Background(), ti.client, ti.namespace, status, nil, "my-cool-configmap", time.Date(2023, 11, 24, 4, 28, 19, 546750398, time.UTC))
	if err != nil {
