...
func getBasicMpa() *mpa_types.MultidimPodAutoscaler {
	return mpa_test.MultidimPodAutoscaler().WithContainer("any").Get()
}
...
func TestEvictReplicatedByController(t *testing.T) {
	...
	minReplicas := int32(1)
	mpaSingleReplica.Spec.Constraints = &mpa_types.ScalingConstraints{
		Global: &mpa_types.HorizontalScalingConstraints{MinReplicas: &minReplicas},
	}
	...
	testCases := []struct {
		...
		{
			name:               "Can evict even a single Pod using HorizontalScalingConstraints.MinReplicas.",
			replicas:           1,
			evictionTollerance: 0.5,
			mpa:                mpaSingleReplica,
			pods: []podWithExpectations{
				{
					pod:             generatePod().Get(),
					canEvict:        true,
	...
	for _, testCase := range testCases {
		...
		eviction := factory.NewPodsEvictionRestriction(pods, testCase.mpa)
...
func TestEvictReplicatedByReplicaSet(t *testing.T) {
	...
	eviction := factory.NewPodsEvictionRestriction(pods, getBasicMpa())
