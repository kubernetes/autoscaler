...
func TestSortPriority(t *testing.T) {
	...
	mpa := mpa_test.MultidimPodAutoscaler().WithContainer(containerName).WithTarget("10", "").Get()
	...
	calculator := NewUpdatePriorityCalculator(mpa, nil, &mpa_test.FakeRecommendationProcessor{}, priorityProcessor)
...
func TestSortPriorityResourcesDecrease(t *testing.T) {
	...
	mpa := mpa_test.MultidimPodAutoscaler().WithContainer(containerName).WithTarget("5", "").Get()
...
func TestUpdateNotRequired(t *testing.T) {
	pod1 := test.Pod().WithName("POD1").AddContainer(test.Container().WithName(containerName).WithCPURequest(resource.MustParse("4")).Get()).Get()
	mpa := mpa_test.MultidimPodAutoscaler().WithContainer(containerName).WithTarget("4", "").Get()

	priorityProcessor := NewFakeProcessor(map[string]PodPriority{"POD1": {
		ResourceDiff: 0.0,
	}})
	calculator := NewUpdatePriorityCalculator(mpa, nil, &mpa_test.FakeRecommendationProcessor{},
		priorityProcessor)
...
func TestUseProcessor(t *testing.T) {
	...
	recommendationProcessor := &mpa_test.RecommendationProcessorMock{}
	recommendationProcessor.On("Apply").Return(processedRecommendation, nil)

	mpa := mpa_test.MultidimPodAutoscaler().WithContainer(containerName).WithTarget("5", "5M").Get()
	...
	calculator := NewUpdatePriorityCalculator(
		mpa, nil, recommendationProcessor, priorityProcessor)
...
func TestUpdateLonglivedPods(t *testing.T) {
	...
	mpa := mpa_test.MultidimPodAutoscaler().WithContainer(containerName).
		WithTarget("5", "").
		WithLowerBound("1", "").
	...
	calculator := NewUpdatePriorityCalculator(
		mpa, &UpdateConfig{MinChangePriority: 0.5}, &mpa_test.FakeRecommendationProcessor{}, priorityProcessor)
...
func TestDontUpdatePodWithQuickOOMNoResourceChange(t *testing.T) {
	pod := test.Pod().WithName("POD1").AddContainer(test.Container().WithName(containerName).WithCPURequest(resource.MustParse("2")).WithMemRequest(resource.MustParse("8Gi")).Get()).Get()
	...
	calculator := NewUpdatePriorityCalculator(
		mpa, &UpdateConfig{MinChangePriority: 0.1}, &mpa_test.FakeRecommendationProcessor{}, priorityProcessor)
...
func TestQuickOOM_ContainerResourcePolicy(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(fmt.Sprintf("test case: %s", tc.name), func(t *testing.T) {
			...
			mpa.Spec.ResourcePolicy = &vpa_types.PodResourcePolicy{
...
func TestNoPods(t *testing.T) {
	calculator := NewUpdatePriorityCalculator(nil, nil, &mpa_test.FakeRecommendationProcessor{},
...
func (p *pod1Admission) LoopInit([]*apiv1.Pod, map[*mpa_types.MultidimPodAutoscaler][]*apiv1.Pod) {}
...
func TestAdmission(t *testing.T) {
	...
	calculator := NewUpdatePriorityCalculator(mpa, nil,
		&mpa_test.FakeRecommendationProcessor{}, priorityProcessor)
