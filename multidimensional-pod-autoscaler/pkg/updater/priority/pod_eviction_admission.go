...
type PodEvictionAdmission interface {
	...
	// pods currently controlled by MPA in this cluster.
	LoopInit(allLivePods []*apiv1.Pod, mpaControlledPods map[*mpa_types.MultidimPodAutoscaler][]*apiv1.Pod)
	...
	// when no MPA objects are present in the cluster.
	CleanUp()
}
...
func (a *sequentialPodEvictionAdmission) LoopInit(allLivePods []*apiv1.Pod, mpaControlledPods map[*mpa_types.MultidimPodAutoscaler][]*apiv1.Pod) {
	for _, admission := range a.admissions {
		admission.LoopInit(allLivePods, mpaControlledPods)
	}
}
...
func (n *noopPodEvictionAdmission) LoopInit(allLivePods []*apiv1.Pod, mpaControlledPods map[*mpa_types.MultidimPodAutoscaler][]*apiv1.Pod) {
}
