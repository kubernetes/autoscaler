...
// NewUpdatePriorityCalculator creates new UpdatePriorityCalculator for the given MPA object
// an update config.
// If the MPA resource policy is nil, there will be no policy restriction on update.
// If the given update config is nil, default values are used.
func NewUpdatePriorityCalculator(mpa *mpa_types.MultidimPodAutoscaler,
	config *UpdateConfig,
	recommendationProcessor mpa_api_util.RecommendationProcessor,
...
func (calc *UpdatePriorityCalculator) AddPod(pod *apiv1.Pod, now time.Time) {
	processedRecommendation, _, err := calc.recommendationProcessor.Apply(calc.mpa.Status.Recommendation, calc.mpa.Spec.ResourcePolicy, calc.mpa.Status.Conditions, pod)
	if err != nil {
	...
	updatePriority := calc.priorityProcessor.GetUpdatePriority(pod, calc.mpa, processedRecommendation)
	...
	for i := range pod.Status.ContainerStatuses {
		...
		crp := vpa_api_util.GetContainerResourcePolicy(cs.Name, calc.mpa.Spec.ResourcePolicy)
	...
	// - a MPA scaled container OOMed in less than evictAfterOOMThreshold.
	if !updatePriority.OutsideRecommendedRange && !quickOOM {
		if pod.Status.StartTime == nil {
			// TODO: Set proper condition on the MPA.
...
func parseVpaObservedContainers(pod *apiv1.Pod) (bool, sets.String) {
	...
	if hasObservedContainers {
		if containers, err := annotations.ParseVpaObservedContainersValue(observedContainers); err != nil {
			klog.Errorf("MPA annotation %s failed to parse: %v", observedContainers, err)
