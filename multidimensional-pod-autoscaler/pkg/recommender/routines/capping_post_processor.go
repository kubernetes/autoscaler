...
func (c CappingPostProcessor) Process(mpa *mpa_types.MultidimPodAutoscaler, recommendation *vpa_types.RecommendedPodResources) *vpa_types.RecommendedPodResources {
	// TODO: maybe rename the vpa_utils.ApplyVPAPolicy to something that mention that it is doing capping only
	cappedRecommendation, err := vpa_utils.ApplyVPAPolicy(recommendation, mpa.Spec.ResourcePolicy)
	if err != nil {
		klog.ErrorS(err, "Failed to apply policy for VPA", "vpa", klog.KObj(mpa))
