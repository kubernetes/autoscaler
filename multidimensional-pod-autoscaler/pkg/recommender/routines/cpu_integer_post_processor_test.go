...
func TestExtractContainerName(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "real",
			key:    mpaPostProcessorPrefix + "kafka" + mpaPostProcessorIntegerCPUSuffix,
			prefix: mpaPostProcessorPrefix,
			suffix: mpaPostProcessorIntegerCPUSuffix,
			want:   "kafka",
		},
	}
...
func TestIntegerCPUPostProcessor_Process(t *testing.T) {
	tests := []struct {
			...
			mpa: &mpa_types.MultidimPodAutoscaler{
				ObjectMeta: metav1.ObjectMeta{
					Annotations: map[string]string{
						mpaPostProcessorPrefix + "container-other" + mpaPostProcessorIntegerCPUSuffix: mpaPostProcessorIntegerCPUValue,
					}}},
			recommendation: &vpa_types.RecommendedPodResources{
		...
		},
		{
			...
			mpa: &mpa_types.MultidimPodAutoscaler{
				ObjectMeta: metav1.ObjectMeta{Annotations: map[string]string{
					mpaPostProcessorPrefix + "container1" + mpaPostProcessorIntegerCPUSuffix: mpaPostProcessorIntegerCPUValue,
				}}},
			recommendation: &vpa_types.RecommendedPodResources{
				ContainerRecommendations: []vpa_types.RecommendedContainerResources{
					test.Recommendation().WithContainer("container1").WithTarget("8.6", "200Mi").GetContainerResources(),
		...
		},
		{
			...
			mpa: &mpa_types.MultidimPodAutoscaler{
				ObjectMeta: metav1.ObjectMeta{Annotations: map[string]string{
					mpaPostProcessorPrefix + "container1" + mpaPostProcessorIntegerCPUSuffix: mpaPostProcessorIntegerCPUValue,
					mpaPostProcessorPrefix + "container2" + mpaPostProcessorIntegerCPUSuffix: mpaPostProcessorIntegerCPUValue,
				}}},
			recommendation: &vpa_types.RecommendedPodResources{
				ContainerRecommendations: []vpa_types.RecommendedContainerResources{
					test.Recommendation().WithContainer("container1").WithTarget("8.6", "200Mi").GetContainerResources(),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := c.Process(tt.mpa, tt.recommendation)
			assert.True(t, equalRecommendedPodResources(tt.want, got), "Process(%v, %v, nil)", tt.mpa, tt.recommendation)
		})
	}
}
