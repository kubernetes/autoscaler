...
const (
	...
	mpaPostProcessorPrefix           = "mpa-post-processor.kubernetes.io/"
	mpaPostProcessorIntegerCPUSuffix = "_integerCPU"
	mpaPostProcessorIntegerCPUValue  = "true"
)
...
func (p *IntegerCPUPostProcessor) Process(mpa *mpa_types.MultidimPodAutoscaler, recommendation *vpa_types.RecommendedPodResources) *vpa_types.RecommendedPodResources {

	amendedRecommendation := recommendation.DeepCopy()

	for key, value := range mpa.Annotations {
		containerName := extractContainerName(key, mpaPostProcessorPrefix, mpaPostProcessorIntegerCPUSuffix)
		if containerName == "" || value != mpaPostProcessorIntegerCPUValue {
			continue
		}
