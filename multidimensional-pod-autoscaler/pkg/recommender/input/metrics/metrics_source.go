...
func (s *externalMetricsClient) List(ctx context.Context, namespace string, opts v1.ListOptions) (*v1beta1.PodMetricsList, error) {
	...
	for _, mpa := range s.clusterState.Mpas {
		if mpa.PodCount == 0 {
			continue
		}

		if namespace != "" && mpa.ID.Namespace != namespace {
			continue
		}

		nsClient := s.externalClient.NamespacedMetrics(mpa.ID.Namespace)
		pods := s.clusterState.GetMatchingPods(mpa)
		...
		for _, pod := range pods {
			...
			selector := mpa.PodSelector.Add(*podNameReq)
			podMets := v1beta1.PodMetrics{
				TypeMeta:   v1.TypeMeta{},
				ObjectMeta: v1.ObjectMeta{Namespace: mpa.ID.Namespace, Name: pod.PodName},
				Window:     v1.Duration{},
			...
			for resourceName, metricName := range s.options.ResourceMetrics {
				...
				if m == nil || len(m.Items) == 0 {
					klog.V(4).InfoS("External Metrics Query for MPA: No items", "mpa", klog.KRef(mpa.ID.Namespace, mpa.ID.MpaName), "resource", resourceName, "metric", metricName)
					continue
				}
				klog.V(4).InfoS("External Metrics Query for MPA", "mpa", klog.KRef(mpa.ID.Namespace, mpa.ID.MpaName), "resource", resourceName, "metric", metricName, "itemCount", len(m.Items), "firstItem", m.Items[0])
