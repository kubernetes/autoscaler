...
func TestSelfRegistrationBase(t *testing.T) {
	...
	assert.Equal(t, "mpa.k8s.io", webhook.Name, "expected webhook name to match")
	...
	MPARule := webhook.Rules[1]
	assert.Equal(t, []admissionregistration.OperationType{admissionregistration.Create, admissionregistration.Update}, MPARule.Operations, "expected operations to match")
	assert.Equal(t, []string{"autoscaling.k8s.io"}, MPARule.APIGroups, "expected API groups to match")
	assert.Equal(t, []string{"*"}, MPARule.APIVersions, "ehook.Rulxpected API versions to match")
	assert.Equal(t, []string{"verticalpodautoscalers"}, MPARule.Resources, "expected resources to match")
