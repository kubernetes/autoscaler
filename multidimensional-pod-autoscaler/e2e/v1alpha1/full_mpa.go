...
var _ = FullMpaE2eDescribe("Pods under MPA", func() {
	var (
		rc *ResourceConsumer
	)
	replicas := 3
	...
	ginkgo.BeforeEach(func() {
		...
		minReplicas := int32(1)
		maxReplicas := int32(2)
		hconstraints := mpa_types.HorizontalScalingConstraints{
		...
		mpaCRD := test.MultidimPodAutoscaler().
			...
			WithHorizontalScalingConstraints(hconstraints).
...
var _ = FullMpaE2eDescribe("Pods under MPA with default recommender explicitly configured", func() {
	var (
		rc *ResourceConsumer
	)
	replicas := 3
	...
	ginkgo.BeforeEach(func() {
		...
		ginkgo.By("Setting up a MPA CRD with Recommender explicitly configured")
		targetRef := &autoscaling.CrossVersionObjectReference{
			APIVersion: "apps/v1",
			Kind:       "Deployment",
			Name:       "hamster",
...
var _ = FullMpaE2eDescribe("Pods under MPA with non-recognized recommender explicitly configured", func() {
	var (
		rc *ResourceConsumer
	)
	replicas := 3
...
var _ = FullMpaE2eDescribe("OOMing pods under MPA", func() {
	const replicas = 3
