...
const (
	...
	fullMpaSuite                 = "full-mpa"
	actuationSuite               = "actuation"
	pollInterval                 = 10 * time.Second
	pollTimeout                  = 15 * time.Minute
	cronJobsWaitTimeout          = 15 * time.Minute
	// MpaEvictionTimeout is a timeout for MPA to restart a pod if there are no
	// mechanisms blocking it (for example PDB).
	MpaEvictionTimeout = 3 * time.Minute
...
// E2eDescribe describes a MPA e2e test.
func E2eDescribe(scenario, name string, body func()) bool {
	return SIGDescribe(fmt.Sprintf("[MPA] [%s] [v1] %s", scenario, name), body)
}

// RecommenderE2eDescribe describes a MPA recommender e2e test.
func RecommenderE2eDescribe(name string, body func()) bool {
	return E2eDescribe(recommenderComponent, name, body)
}

// UpdaterE2eDescribe describes a MPA updater e2e test.
func UpdaterE2eDescribe(name string, body func()) bool {
	return E2eDescribe(updateComponent, name, body)
}

// AdmissionControllerE2eDescribe describes a MPA admission controller e2e test.
func AdmissionControllerE2eDescribe(name string, body func()) bool {
	return E2eDescribe(admissionControllerComponent, name, body)
}

// FullMpaE2eDescribe describes a MPA full stack e2e test.
func FullMpaE2eDescribe(name string, body func()) bool {
	return E2eDescribe(fullMpaSuite, name, body)
}

// ActuationSuiteE2eDescribe describes a MPA actuation e2e test.
...
func getMpaClientSet(f *framework.Framework) mpa_clientset.Interface {
	config, err := framework.LoadConfig()
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "unexpected error loading framework")
	return mpa_clientset.NewForConfigOrDie(config)
}

// InstallMPA installs a MPA object in the test cluster.
func InstallMPA(f *framework.Framework, mpa *mpa_types.MultidimPodAutoscaler) {
	mpaClientSet := getMpaClientSet(f)
	_, err := mpaClientSet.AutoscalingV1alpha1().MultidimPodAutoscalers(f.Namespace.Name).Create(context.TODO(), mpa, metav1.CreateOptions{})
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "unexpected error creating MPA")
	// apiserver ignore status in mpa create, so need to update status
	if !isStatusEmpty(&mpa.Status) {
		if mpa.Status.Recommendation != nil {
			PatchMpaRecommendation(f, mpa, mpa.Status.Recommendation)
		}
	}
}

func isStatusEmpty(status *mpa_types.MultidimPodAutoscalerStatus) bool {
	if status == nil {
		return true
	}
...
// InstallRawMPA installs a MPA object passed in as raw json in the test cluster.
func InstallRawMPA(f *framework.Framework, obj interface{}) error {
	mpaClientSet := getMpaClientSet(f)
	err := mpaClientSet.AutoscalingV1alpha1().RESTClient().Post().
...
// PatchMpaRecommendation installs a new recommendation for MPA object.
func PatchMpaRecommendation(f *framework.Framework, mpa *mpa_types.MultidimPodAutoscaler,
	...
	gomega.Expect(err).NotTo(gomega.HaveOccurred())
	_, err = getMpaClientSet(f).AutoscalingV1alpha1().MultidimPodAutoscalers(f.Namespace.Name).Patch(context.TODO(), mpa.Name, types.JSONPatchType, bytes, metav1.PatchOptions{}, "status")
	gomega.Expect(err).NotTo(gomega.HaveOccurred(), "Failed to patch MPA.")
}
...
// WaitForMPAMatch pools MPA object until match function returns true. Returns
// polled mpa object. On timeout returns error.
func WaitForMPAMatch(c mpa_clientset.Interface, mpa *mpa_types.MultidimPodAutoscaler, match func(mpa *mpa_types.MultidimPodAutoscaler) bool) (*mpa_types.MultidimPodAutoscaler, error) {
	var polledMpa *mpa_types.MultidimPodAutoscaler
	err := wait.PollUntilContextTimeout(context.Background(), pollInterval, pollTimeout, true, func(ctx context.Context) (done bool, err error) {
		polledMpa, err = c.AutoscalingV1alpha1().MultidimPodAutoscalers(mpa.Namespace).Get(context.TODO(), mpa.Name, metav1.GetOptions{})
		if err != nil {
			return false, err
		}
	...
	if err != nil {
		return nil, fmt.Errorf("error waiting for recommendation present in %v: %v", mpa.Name, err)
	}
	return polledMpa, nil
}

// WaitForRecommendationPresent pools MPA object until recommendations are not empty. Returns
// polled mpa object. On timeout returns error.
func WaitForRecommendationPresent(c mpa_clientset.Interface, mpa *mpa_types.MultidimPodAutoscaler) (*mpa_types.MultidimPodAutoscaler, error) {
	return WaitForMPAMatch(c, mpa, func(mpa *mpa_types.MultidimPodAutoscaler) bool {
		return mpa.Status.Recommendation != nil && len(mpa.Status.Recommendation.ContainerRecommendations) != 0
	})
}

// WaitForUncappedCPURecommendationAbove pools MPA object until uncapped recommendation is above specified value.
// Returns polled MPA object. On timeout returns error.
func WaitForUncappedCPURecommendationAbove(c mpa_clientset.Interface, mpa *mpa_types.MultidimPodAutoscaler, minMilliCPU int64) (*mpa_types.MultidimPodAutoscaler, error) {
	return WaitForMPAMatch(c, mpa, func(mpa *mpa_types.MultidimPodAutoscaler) bool {
		if mpa.Status.Recommendation == nil || len(mpa.Status.Recommendation.ContainerRecommendations) == 0 {
			return false
		}
		uncappedCpu := mpa.Status.Recommendation.ContainerRecommendations[0].UncappedTarget[apiv1.ResourceCPU]
