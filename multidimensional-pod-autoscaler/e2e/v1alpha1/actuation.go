...
import (
	...
	"k8s.io/autoscaler/multidimensional-pod-autoscaler/e2e/utils"
	"k8s.io/autoscaler/multidimensional-pod-autoscaler/pkg/utils/annotations"
...
var _ = ActuationSuiteE2eDescribe("Actuation", func() {
	...
	ginkgo.It("stops when pods get pending", func() {
		...
		ginkgo.By("Setting up a MPA CRD with ridiculous request")
	...
	ginkgo.It("never applies recommendations when update mode is Off", func() {
		...
		ginkgo.By("Setting up a MPA CRD in mode Off")
		containerName := GetHamsterContainerNameByIndex(0)
		mpaCRD := test.MultidimPodAutoscaler().
			WithName("hamster-mpa").
			WithNamespace(f.Namespace.Name).
		...
		ginkgo.By(fmt.Sprintf("Waiting for pods to be evicted, hoping it won't happen, sleep for %s", MpaEvictionTimeout.String()))
	...
	ginkgo.It("applies recommendations only on restart when update mode is Initial", func() {
		...
		ginkgo.By("Setting up a MPA CRD in mode Initial")
		containerName := GetHamsterContainerNameByIndex(0)
		mpaCRD := test.MultidimPodAutoscaler().
			WithName("hamster-mpa").
			WithNamespace(f.Namespace.Name).
	...
	ginkgo.It("observes pod disruption budget", func() {
		...
		ginkgo.By(fmt.Sprintf("Waiting for pods to be evicted, sleep for %s", MpaEvictionTimeout.String()))
	...
	ginkgo.It("does not act on injected sidecars", func() {
		...
		// Webhook must be placed after mpa webhook. Webhooks are registered alphabetically.
		...
		ginkgo.By("Setting up a hamster mpa")
		container1Name := GetHamsterContainerNameByIndex(0)
		container2Name := GetHamsterContainerNameByIndex(1)
		mpaCRD := test.MultidimPodAutoscaler().
			WithName("hamster-mpa").
		...
		for _, pod := range podList.Items {
			observedContainers, ok := pod.GetAnnotations()[annotations.MpaObservedContainersLabel]
			gomega.Expect(ok).To(gomega.Equal(true))
			containers, err := annotations.ParseMpaObservedContainersValue(observedContainers)
			gomega.Expect(err).NotTo(gomega.HaveOccurred())
...
func testDoesNotEvictSingletonPodByDefault(f *framework.Framework, controller *autoscaling.CrossVersionObjectReference) {
	...
	// No eviction is expected with the default settings of MPA object
	ginkgo.By(fmt.Sprintf("Waiting for pods to be evicted, hoping it won't happen, sleep for %s", MpaEvictionTimeout.String()))
	CheckNoPodsEvicted(f, MakePodSet(podList))
}
...
func testEvictsSingletonPodWhenConfigured(f *framework.Framework, controller *autoscaling.CrossVersionObjectReference) {
	...
	// Prepare the MPA to allow single-Pod eviction.
	ginkgo.By("Setting up a MPA CRD")
	containerName := GetHamsterContainerNameByIndex(0)
	mpaCRD := test.MultidimPodAutoscaler().
		WithName("hamster-mpa").
