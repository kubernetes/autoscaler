...
var _ = AdmissionControllerE2eDescribe("Admission-controller", func() {
	...
	ginkgo.It("starts pods with default request when recommendation includes an extra container when a limit range applies", func() {
		...
		// should change it to recommended 250m CPU and 200Mi of memory.
		for _, pod := range podList.Items {
			// This is a bug; MPA should behave here like it does without a limit range
	...
	ginkgo.It("doesn't block patches", func() {
		...
		PatchMpaRecommendation(f, mpaCRD, &vpa_types.RecommendedPodResources{
	...
	ginkgo.It("caps request to max set in MPA", func() {
		d := NewHamsterDeploymentWithResources(f, ParseQuantityOrDie("100m") /*cpu*/, ParseQuantityOrDie("100Mi") /*memory*/)
		...
		// capped to max specified in MPA)
		for _, pod := range podList.Items {
	...
	ginkgo.It("raises request to min set in MPA", func() {
		d := NewHamsterDeploymentWithResources(f, ParseQuantityOrDie("100m") /*cpu*/, ParseQuantityOrDie("100Mi") /*memory*/)
		...
		// recommendation raised to min specified in MPA)
		for _, pod := range podList.Items {
	...
	ginkgo.It("leaves users request when no recommendation", func() {
		...
		// MPA has no recommendation, so user's request is passed through
		for _, pod := range podList.Items {
			gomega.Expect(pod.Spec.Containers[0].Resources.Requests[apiv1.ResourceCPU]).To(gomega.Equal(ParseQuantityOrDie("100m")))
			gomega.Expect(pod.Spec.Containers[0].Resources.Requests[apiv1.ResourceMemory]).To(gomega.Equal(ParseQuantityOrDie("100Mi")))
		}
	...
	ginkgo.It("passes empty request when no recommendation and no user-specified request", func() {
		...
		// MPA has no recommendation, deployment has no request specified
		for _, pod := range podList.Items {
	...
	ginkgo.It("accepts valid and rejects invalid VPA object", func() {
		...
		validVPA := []byte(`{
			...
			"metadata": {"name": "hamster-mpa-valid"},
			"spec": {
				"targetRef": {
					"apiVersion": "apps/v1",
					"kind": "Deployment",
		...
		invalidVPA := []byte(`{
			"kind": "VerticalPodAutoscaler",
			"apiVersion": "autoscaling.k8s.io/v1",
			"metadata": {"name": "hamster-mpa-invalid"},
			"spec": {
				"targetRef": {
					"apiVersion": "apps/v1",
					"kind": "Deployment",
		...
		err2 := InstallRawMPA(f, invalidVPA)
		gomega.Expect(err2).To(gomega.HaveOccurred(), "Invalid MPA object accepted")
		gomega.Expect(err2.Error()).To(gomega.MatchRegexp(`.*admission webhook .*mpa.* denied the request: .*`))
	})

	ginkgo.It("reloads the webhook certificate", func(ctx ginkgo.SpecContext) {
		ginkgo.By("Retrieving alternative certificate")
		c := f.ClientSet
		e2eCertsSecret, err := c.CoreV1().Secrets(metav1.NamespaceSystem).Get(ctx, "mpa-e2e-certs", metav1.GetOptions{})
		gomega.Expect(err).To(gomega.Succeed(), "Failed to get mpa-e2e-certs secret")
		actualCertsSecret, err := c.CoreV1().Secrets(metav1.NamespaceSystem).Get(ctx, "mpa-tls-certs", metav1.GetOptions{})
		gomega.Expect(err).To(gomega.Succeed(), "Failed to get mpa-tls-certs secret")
		...
		gomega.Expect(err).To(gomega.Succeed(), "Failed to update mpa-tls-certs secret with e2e rotation certs")
		...
		for _, p := range pods.Items {
			if strings.HasPrefix(p.Name, "mpa-admission-controller") {
		...
		err = InstallRawMPA(f, invalidVPA)
		gomega.Expect(err).To(gomega.HaveOccurred(), "Invalid MPA object accepted")
		gomega.Expect(err.Error()).To(gomega.MatchRegexp(`.*admission webhook .*mpa.* denied the request: .*`), "Admission controller did not inspect the object")
	})
...
func startDeploymentPods(f *framework.Framework, deployment *appsv1.Deployment) *apiv1.PodList {
	...
	// Wait after creating deployment to ensure MPA knows about it, then scale up.
